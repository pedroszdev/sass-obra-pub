...
**Varredura completa (16/07/2026) — OWASP, código inteiro:** **nenhuma vulnerabilidade explorável encontrada.** Confirmados sadios: escopo por usuário em todo `:id`, zero SQL cru (os `Raw()` usam parâmetro nomeado), zero sink de XSS no front, `esc()` em todo dado de usuário nos e-mails, `email_verified`+`aud`+`nonce` no vínculo do Google, webhook da Stripe com assinatura sobre corpo cru + idempotência + guarda de ordem, `pnpm audit --prod` limpo, nenhum PII em log. Fechados três pontos de defesa em profundidade (nenhum era buraco): allowlist de `https:` na URL de documento que vem do feed da fonte (`common/url-documento.ts`), `MaxLength(72)` nas senhas de *leitura* (login e senha atual — as de escrita já tinham pelo `@IsSenhaForte`), e `.gitignore` para fatura/recibo da Stripe largados na raiz. **Decisão registrada:** os cookies de repasse do Google seguem `SameSite=None` — trocá-los por `Lax` reintroduz a quebra de login no Safari (§8, T-156); o CSRF de sabotagem que isso deixa aberto é aceito conscientemente.
