...
- **Notificações por e-mail (T-103):** `@Cron` diário **+** disparo manual `POST /notificacoes/run` (mesmo token `CAPTACAO_TRIGGER_TOKEN`) — para um cron externo disparar, já que o `@Cron` hiberna. Manda e-mail-resumo (certidão vencendo/vencida + prazo próximo) só a usuários com e-mail **verificado + toggle ligado**, sem duplicar (tabela `notification_log`). WhatsApp fica pendente de provedor.
