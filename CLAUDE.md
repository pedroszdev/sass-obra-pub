...
- **Cancelamento no Portal (T-144, correção 15/07):** quem cancela no Customer Portal fica com status `active` + `cancel_at_period_end: true` na Stripe — ela NÃO muda para `canceled` (o status só vira canceled no fim do período). O mapper lê essa flag (`cancelAtPeriodEnd` na entidade/resposta) e a tela mostra "Cancelada · acesso até X" em vez de "renova em X". Sem isso, nada mudava na plataforma ao cancelar.
