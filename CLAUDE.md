...
- **Login com Google por redirect (T-126b):** entrar/cadastrar não abre mais popup — o Google navega a página e faz POST do `id_token` para `POST /auth/google/callback` (na **API**), que cria a sessão (cookie de refresh) e devolve o navegador em `WEB_ORIGIN/entrando`. Exige cadastrar no Google Cloud Console, como **Authorized redirect URI**, `http://localhost:3000/auth/google/callback` (dev) e `https://obrapub-api.onrender.com/auth/google/callback` (prod) — sem isso o Google recusa o fluxo. O anti-CSRF **não** é o `g_csrf_token` do Google (ele não funciona com o callback em outro domínio): é um **nonce** da própria API (`GET /auth/google/inicio` → cookie `obrapub_gnonce`, conferido dentro do id_token). O popup sobrevive só no Perfil, para reautenticar na exclusão de conta (`POST /auth/google`, JSON).
