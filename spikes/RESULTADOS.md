...
**O que este número NÃO mede (limite honesto):** ele pega **falso positivo** (IA inventa exigência) mas **não pega falso negativo** (IA deixa passar exigência que existe). E falso negativo é justamente o erro que gera o **"apto" indevido** — o pior erro do produto. Fechar isso exige rotular à mão o conjunto de exigências de uma amostra → **task T-139**. `OUTRA` é aberta por definição e fica fora da conta, em vez de contar como alucinação que não sabemos medir.

### Achados (investigados depois — 2 dos 3 eram erro de análise, não defeito)

1. **Documento errado vencia o edital → T-137 (corrigido).** O `NAO_EDITAL` não filtrava `apêndice`/`as built`, e `temSinalHabilitacao` aceitava `certidao`+`cnpj` como sinal. Um `9_Apendice_As_Built_VF.pdf` era lido no lugar do edital, dando **zero exigências cacheadas para sempre** (`getOrExtract` só reprocessa `erro`). Afetava 1 dos 25.
2. **Linha-lixo por coluna desalinhada → T-136 (corrigido).** ~6 linhas saíram com a **descrição igual à unidade** (`"UNID."`, `"M2"`, `"M"`) — passavam em qualquer guarda de "não vazio" e entrariam na proposta do empreiteiro. `filtrarItensUteis` passou a descartá-las (71 → 65 itens no edital real, nenhum item bom perdido).
3. ~~**`scorePlanilhaNome` escolhe o template EM BRANCO**~~ — **não era defeito.** A `PLANILHA_PREENCHIMENTO` era a **única** planilha daquele edital; ela traz descrição + unidade + quantidade (o preço é o licitante que preenche). Excluí-la daria **zero itens**. Quando existe `PLANILHA_ORCAMENTARIA`, ela já vence (score 3 × 1). *(O "preço ausente em todos" foi artefato do script de inspeção, que lia `precoUnitario` em vez de `precoReferencia`.)*
4. ~~**Sub-extração no `ORCAMENTO OFICIAL` (3 itens)**~~ — **não era defeito.** O PDF é uma *"PLANILHA REFERENCIAL — PROJETOS DE ENGENHARIA"* de 4.157 chars: o edital contrata **projetos**, não obra. Três itens é o que existe.

> **Padrão que se repete nesta medição:** três das cinco "anomalias" (as 7 alucinações, o template em branco, a sub-extração) eram **defeito do instrumento de medida**, não do sistema medido. Vale mais conferir o detector antes de acusar a IA.
