...
- [x] **T-95 — Home ("Melhor obra pra você hoje" + "Obras pra você hoje") prioriza aptidão + município** 🟢
  ...
  - **Feito (2026-07-07) — front-only (`HomePage.tsx`):** duas buscas — uma UF-wide `pageSize:1` só para `regiaoCount`/"Ver todas" (mantidos na UF inteira, como o escopo pede) e um **pool `pageSize:20`** filtrado pelos municípios preferidos (`codigoIbge`, T-94) quando houver. Ranqueio estável por `aptoRank` (veredito `apto` → 0, resto → 1) sobre o pool já ordenado por recência: **apto primeiro, recência como desempate**; sem apto → recência pura; sem município → UF inteira (idêntico ao anterior). Destaque = `ranked[0]`, lista = `ranked.slice(1,4)`. Estados de loading/erro/vazio do destaque+lista passam a olhar o `poolState`. Sem backend novo. Build/lint/vitest (33) verdes. ⚠️ Sign-off no navegador pendente (sem Postgres no ar aqui).
...
- [x] **T-96 — Ordenar e limitar o "Precisa da sua atenção" da Home** 🟢
  ...
  - **Feito (2026-07-08) — front-only:** cada card do bloco ganhou uma chave numérica `ordem` (dias até vencer/entregar; **vencidos negativos no topo**, rascunho `+Infinity` no fim); o bloco passa a **ordenar por `ordem`** cruzando as 4 categorias e a **capar em `ATENCAO_LIMITE` (4)** — o excedente vira o link **"Ver tudo (N) → /alertas"** (decisão: Central de Alertas). **Janelas NÃO centralizadas de propósito:** a da Home (prazo ≤7d, client) e a dos Alertas (≤14d, backend) vivem em camadas diferentes e não podem compartilhar uma constante hoje (dívida §10.4 — tipos/constantes compartilhados ainda não moram em `packages/`); documentado no código. Build/lint/vitest verdes. ⚠️ Sign-off no navegador pendente (§4.4).
  - **Pronto quando:** os itens do bloco aparecem em ordem de urgência real (o mais crítico no topo, independente da categoria), a lista é capada com "ver tudo", e a lógica de janela de prazo/certidão vive num só lugar reusado pela Home e pelos Alertas. ✅ *(a centralização das janelas ficou como decisão consciente adiada — ver acima)*
- [x] **T-97 — Stat cards da Home: eliminar destino/métrica duplicados** 🟢
  ...
  - **Feito (2026-07-08) — front-only:** fundidos "Prontidão do perfil" + "Documentos válidos" num só card (`%` principal + subtexto "N/M certidões válidas" via novo prop `sub` do `StatCard`; segue → `/documentos`). Slot liberado virou **"Propostas em andamento"** → `/orcamentos` (contagem de propostas **vivas** = rascunho + enviada, excluindo ganhou/nao_ganhou; do `usePropostas` já carregado, sem fetch novo). Os 4 cards agora levam a 4 destinos distintos. Build/lint/vitest verdes. ⚠️ Sign-off no navegador pendente (§4.4).
...
- [x] **T-126 — Criação de conta e login com Google (OAuth)** 🔴 **(A)**
  ...
  - **Feito (2026-07-09) — dep aprovada: `google-auth-library`; decisões do dono: vincular por e-mail, checkbox de aceite antes do botão, re-auth Google para excluir conta:**
    - **Backend:** migration `AddGoogleAuthToUsers` (à mão, §10.1) — `password_hash` nullable, `provider` (`local`|`google`), `google_sub` único quando presente. `GoogleVerifierService` (adapter fino sobre `OAuth2Client`) verifica assinatura/**audiência**/expiração e **exige `email_verified`**; sem `GOOGLE_CLIENT_ID` → **503** (degrada como IA/SMTP, §8). `POST /auth/google` reusa `entregarSessao` (cookie httpOnly, T-119a) + throttle AUTH (T-104).
    - **Três caminhos:** `google_sub` conhecido → loga; **e-mail** conhecido → **vincula** (a senha local continua valendo, `provider` não muda); nada bate → cadastra com `passwordHash: null`, **`emailVerifiedAt = now`** (o Google atesta — fecha a T-132) e **sem UF**.
    - **Aceite LGPD (T-102):** exigido só quando a conta é nova (`aceiteTermos` no DTO; 400 sem ele). O botão do Google no cadastro fica bloqueado até marcar o checkbox; no login, um 400 manda a pessoa para `/cadastro`.
    - **Guardas nos caminhos de senha:** `login`, `change-password` e `forgot-password` recusam conta sem `passwordHash` **antes** do `bcrypt.compare` (que estouraria 500 com hash null). `forgot-password` **não envia** e-mail para conta só-Google (seria *criar* senha, não recuperar) mantendo o 204 anti-enumeração.
    - **Exclusão de conta (T-102):** `DELETE /users/me` aceita **senha OU `idToken`**. Verificar o token não basta — o `sub` precisa bater com o `google_sub` **daquela** conta (um id_token legítimo de outra pessoa passa na verificação). Teste dedicado cobre isso.
    - **UF (o furo que a task não previa):** a `OnboardingPage` mostrava a UF **read-only** (`user.uf`), então conta Google cairia num onboarding sem região e a captação (T-18) nunca rodaria. Onboarding ganhou **`Select` de UF** quando `user.uf == null` + **`PUT /users/me/uf`**; login/cadastro roteiam para `/onboarding` quando falta UF.
    - **Front:** `lib/google.ts` (carrega o SDK do Google sob demanda, **sem dep npm**) + `GoogleButton`; botões em Login/Cadastro; `provider` não vaza — a resposta expõe **`temSenha`/`googleVinculado`**, que é o que a UI decide (Perfil→Segurança esconde "trocar senha" sem senha; exclusão pede o Google). `VITE_GOOGLE_CLIENT_ID` ausente → o botão **não renderiza**.
    - **Testes (+21):** `google-verifier.service.spec` (novo: 503 sem client id, exige audiência, recusa e-mail não verificado, token inválido → 401), `auth.service.spec` (loga por sub / vincula por e-mail / cadastra sem senha e sem UF / recusa cadastro sem aceite / não exige aceite de quem já tem conta / login por senha em conta sem senha), `users.service.spec` (exclusão por Google, **id_token de outra conta → 401**), `api.test` (contratos). API **421→438** verdes; front **33→37**; lint/build limpos nos dois. Grafo de DI verificado carregando os módulos compilados (o `GoogleAuthModule` existe para `users` e `auth` compartilharem o verificador sem ciclo).
    - ✅ **Verificado ao vivo (09/07/2026, Postgres real):** a migration rodou (junto das 5 pendentes de T-99/T-101/T-102/T-132/T-103) — `password_hash` nullable, enum `provider`, `UQ_users_google_sub`, e o **índice GIN sobreviveu** (§10.1). API sobe com o grafo de DI real (`GoogleAuthModule` sem ciclo), `POST /auth/google` e `PUT /users/me/uf` mapeados. Caminhos exercitados por curl: id_token forjado → **401** (não 500), corpo sem `idToken` → 400, login por senha em conta com `password_hash NULL` → **401** (o furo que estouraria 500), `forgot-password` em conta só-Google → 204 **sem gerar token**, e o controle positivo (conta com senha → 204 **com** token). Usuários de teste removidos.
    - ⚠️ **Sign-off no navegador pendente** (§4.4): falta o clique real no botão do Google (exige consentimento humano). O client id OAuth já está nos dois `.env` locais; **origens JS autorizadas** devem conter `http://localhost:5173` (e a origem de prod quando existir); **URIs de redirecionamento ficam vazios** — o fluxo GIS devolve o `id_token` por popup, não por redirect.
  - **Pronto quando:** um usuário cria conta/loga com Google, cai no onboarding se novo, e usa o produto normalmente. ✅ *(código completo; falta credencial OAuth do dono + clique-a-clique)*
...
- [x] **T-104 — Hardening de segurança do backend** 🔴 **(B)**
  ...
  - **Dependência:** —.
  - **Feito (2026-07-07) — dep aprovada: `@nestjs/throttler`:**
    - ✅ **(a) Rate limiting** (mapa ampliado além dos 3 de auth, decisão do dono): `ThrottlerModule` + `ThrottlerGuard` global (100/min por IP, rede de segurança) + `trust proxy` no `main.ts` (senão o IP vira o do proxy do Render). Overrides por rota em `common/throttling/throttle.config.ts`. **Login com dois eixos** (o que estourar primeiro barra): por IP (guard global) contra spraying/CPU do bcrypt **e por email** (`EmailThrottlerGuard`, hash do email) contra brute-force de conta via IPs rotativos — 5/min cada. `register` 5/min (IP; email do atacante muda), `refresh` 10/min (IP, folga p/ cold start), `change-password` 5/min por **usuário** (`UserThrottlerGuard`). **Custo de IA** (§3.4): `exigencias`, `itens-extraidos`, `diagnostico/:editalId`, `propostas/:id/itens/importar` → 30/min por usuário (a 1ª chamada gasta OpenAI). **Upload** de certidão 20/min e `captacao/run` 10/min. `/health` **isento** (`@SkipThrottle`) — keep-alive externo não pode tomar 429.
    - ✅ **(b) Cap no download de PDF:** `documento-texto.service.ts` corta por `Content-Length` e pelo tamanho real (60 MB) → não bufferiza o gigante (espelha o cap do `planilha-texto`).
    - ✅ **(c) Validação de env no boot:** `common/env.validation.ts` (função pura, **sem Joi/dep**) no `ConfigModule.forRoot({ validate })` — boot falha se faltar `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET`; em prod (`NODE_ENV=production`) também `DATABASE_HOST`/`DATABASE_PASSWORD`/`WEB_ORIGIN`. `OPENAI_API_KEY`/`CAPTACAO_TRIGGER_TOKEN` seguem opcionais (degradam com 503).
    - ✅ **(d) Timeout no OpenAI:** `new OpenAI({ timeout: 120s, maxRetries: 2 })` no `ia-extracao.service.ts`.
    - ✅ **(e) Purga de refresh tokens:** `RefreshTokenCleanupService` (`@Cron` diário + uma passada no boot, best-effort) apaga expirados e revogados > 24h. Sem migration.
    - **Testes (+25, suíte 307→332):** `env.validation.spec`, `throttling.guard.spec` (getTracker email/usuário), `refresh-token-cleanup.service.spec`, `documento-texto.service.spec` (cap), e **`throttling.e2e.spec`** — boot real (porta efêmera, sem DB) provando 429 após 5, dimensão email, dimensão IP (anti-spraying), teto global e `/health` isento. Lint limpo. **Verificação ao vivo com DB não feita** (sem Docker/Postgres neste ambiente): o **fluxo cross-origin do cookie de refresh** (herdado da T-119) e o boot com Postgres seguem para sign-off em ambiente com banco.
...
- [x] **T-107 — Revalidar acerto da IA em amostra maior no provider de produção** 🟡 **(B)**
  ...
  - **Feito (2026-07-09) — n=25, `gpt-5.4-mini`, `store: false`. Resultado completo em [`spikes/RESULTADOS.md`](spikes/RESULTADOS.md#t-107):**
    ...
    - **Limite honesto (registrado):** o método pega **falso positivo** (IA inventa exigência), **não pega falso negativo** (IA deixa passar exigência que existe) — e é o falso negativo que gera o **"apto" indevido**. Fechar isso exige rotulagem manual das exigências de uma amostra → **T-139**.
  ...
  - **Achados da medição** (não corrigidos aqui — a T-107 é medir, §4.3): documento errado → **T-137** (feito); linha-lixo na planilha → **T-136** (feito); editais SICAF sem diagnóstico → **T-138**. Dois "achados" iniciais (template em branco, sub-extração) **não eram defeitos** — ver T-136.
...
- [x] **T-137 — Seleção do documento do edital + cache envenenado (achados da T-107)** 🟡 **(B)** *(follow-up da T-107)*
  ...
  - **Feito (2026-07-09) — sem tocar em prompt/schema (não exige remedição, §3.4):**
    ...
    - **Fora de escopo → T-138:** editais **SICAF** (habilitação por registro cadastral) legitimamente não enumeram certidões → hoje viram `indefinido`. Tratá-los exige **campo novo no schema da IA** e, por §3.4, **nova medição de acerto**. Eles serão reprocessados uma vez pela migration e voltarão a ficar vazios (uma chamada desperdiçada, uma vez).
...
- [x] **T-136 — Linha-lixo na planilha de itens (achado da T-107)** 🟢 **(B)** *(follow-up da T-107)*
  - **⚠️ Escopo encolheu na investigação: dos 3 achados registrados na T-107, só 1 era defeito.** Os outros dois foram **erro de análise** (documentado aqui para não voltarem como "bug" no futuro):
    - ~~**(a) Escolhe o template EM BRANCO**~~ — **não é defeito.** No edital `45550167000164-1-000495/2026` a `PLANILHA_PREENCHIMENTO` é a **única** planilha do edital (os outros arquivos são o edital e o extrato). Excluí-la deixaria o empreiteiro com **zero itens**; ela traz descrição + unidade + quantidade, que é exatamente o que a proposta precisa — o `precoReferencia` é null porque é o licitante quem preenche o preço. Quando existe uma `PLANILHA_ORCAMENTARIA` de verdade, ela já vence (score 3 × 1). *Nada a fazer.* (O "precoUnitario ausente em todos" reportado na T-107 foi artefato do script de inspeção, que lia `precoUnitario` em vez de `precoReferencia`.)
    - ~~**(c) Sub-extração no `ORCAMENTO OFICIAL`**~~ — **não é defeito.** O PDF é uma *"PLANILHA REFERENCIAL — PROJETOS DE ENGENHARIA"* de 4.157 chars: o edital contrata **projetos**, não executa obra, e a planilha tem mesmo poucas linhas. Os 3 itens são o que existe. *Nada a fazer.*
  - **(b) Linha-lixo por coluna desalinhada — o defeito real:** em planilha com colunas desalinhadas a IA copia a **unidade para a descrição** (`"UNID."`, `"M2"`, `"M"`). Essas linhas têm letra e quantidade positiva, então passavam pelo `temDescricao` do `filtrarItensUteis` e viravam item na proposta do empreiteiro.
  - **Feito (2026-07-09) — guarda determinística, sem IA nova:**
    - `filtrarItensUteis` (`itens-filtro.ts`, pura) passa a descartar item cuja **descrição é só a unidade**: igual à coluna `unidade` do próprio item, **ou** um token de unidade conhecido (`UNIDADES`), comparando sem acento/caixa/pontuação. Cobre também o caso em que a coluna `unidade` veio vazia e só a descrição carregava o rótulo.
    - **Verificado contra o dado real:** reprocessada a planilha do edital `45550167000164-1-000495/2026` — **71 → 65 itens**, removidas exatamente as **6 linhas-lixo** (`UNID.`/`M2`/`M`), nenhum item real perdido.
    - **Testes (+6):** linhas reais da T-107; unidade vazia; acento/caixa/pontuação (`m³` × `M3`); e a guarda de não-regressão — descrição que **começa** com unidade ("M2 de alvenaria estrutural", "Lote de tubos de concreto") **não** é descartada. API **447→453** verdes, lint limpo.
...
- [x] **T-109 — Fechar lacunas de teste críticas** 🟡 **(C)**
  ...
  - **Feito (2026-07-06):** **Backend** (+18 testes, suíte 307): `itens-extracao.service.spec` (cache/404/indisponível-sem-IA/1-IA-call+extraído/temPlanilha=false/erro/dedup/score-de-planilha); `ia-extracao.service.spec` (custo USD pelo modelo, refusal→erro, sem-conteúdo→erro, truncagem no MAX_CHARS — client injetado via subclasse do `getClient` protected); `planilha-texto.spec` (regex do XLSX: `parseSharedStrings`/`sheetParaTexto` exportados — shared strings, inlineStr, números, células vazias). **Front** (vitest 15→24): `lib/api.test.ts` cobre o cliente HTTP — 401→refresh(cookie)→retry, **coalescência** (2×401 = 1 refresh), rede no refresh **mantém** a sessão, 401 real **desloga** (stub de `localStorage`+`fetch`, node env); `parseNum`/`parseColar` extraídos para `lib/parse-itens.ts` (testável) + `parse-itens.test.ts` (padrão BR, separadores TAB/`;`/2+espaços, descarte de linha sem descrição). **Fora:** teste de componente (`@testing-library/react`) segue como gap — exige dep + jsdom; o "fechamento de 100% do cronograma" já tem cobertura na T-117 (`cronograma.spec`).
  - **Dependência:** —.
  - **Pronto quando:** os pipelines de IA (itens/custo) e o refresh/401 do client têm testes; parsing XLSX e parse de itens cobertos.
- [x] **T-110 — Correções rápidas de front + bugs latentes** 🟢 **(C)**
  ...
  - **Feito (2026-07-06):** ✅ `manifest.webmanifest` → PrumoLicita + `theme_color` `#C25A26` (Âmbar Prumo, casa com o `index.html`). ✅ `AuthProvider` no boot só faz `clearTokens` em **401 real** — blip de rede mantém os tokens (reload recupera). ✅ guarda `frac()` no `ProntidaoPanel` (sem `NaN` quando `total=0`). ✅ `SimpleGrid` do cofre responsivo (`{base:1, xs:3}`). ✅ `diasAteValidade` em **UTC** (sem off-by-one por fuso do servidor). ✅ `reordenarItens` numa **transação** (`manager.transaction`) — sem ordem parcial em falha; teste ajustado. API 289 verdes, front tsc/lint/vitest verdes.
  - **Adiado (com motivo):** `window.confirm` → modal de marca (precisa de `@mantine/modals` — dep a aprovar — ou modal caseiro; não é "barato"); **botões mortos** ("Convidar membro"/"Editar perfil"/"Selecionar arquivos") estão em **telas mockadas** (Perfil/Onboarding — §7 "não mexer fora da task certa"; entram na T-87/T-99/T-108); parsing de data/serial do XLSX (arriscado — merece spike próprio).
  - **Dependência:** —.
  - **Pronto quando:** manifest corrigido, logout só em 401, guardas de null/zero e transação na reordenação; itens de a11y ajustados. ✅ *(itens adiados acima documentados)*
...
- [x] **T-111 — Guia de regularização: transformar "não apto" em "como ficar apto a tempo"** 🟡
  ...
  - **Feito (2026-07-07) — sem IA, sem dep:**
    - **Catálogo** `habilitacao/regularizacao-catalog.ts` (pura `guiaRegularizacao(tipo, uf?)` → `{orgao, url, observacao}`, centralizado §3.3): trio federal (CND→RFB/PGFN, FGTS→Caixa, CNDT→TST) com **link direto** e observação honesta ("emissão imediata **se estiver regular**; havendo pendência, semanas"); ESTADUAL→Sefaz/UF, FALENCIA→TJ/UF, REGISTRO_CONSELHO→CREA/UF e MUNICIPAL→prefeitura com **url null** (evita link quebrado — variam por UF; o órgão já vem resolvido com a UF).
    - **Motores decorados** (cálculo no backend §3.3): `ProntidaoItem`/`DiagnosticoItem` ganham `regularizacao?` só em **pendência** (status ≠ atendido) de certidão/registro; `DiagnosticoEditalResult` ganha **`diasAtePrazo`** (prazo do edital − now, ceil). `ProntidaoInput` ganhou `uf` (carregado do `User.uf` via `UsersService`; `getDiagnosticoEdital` passa `edital.prazoProposta`). **Decisão:** NÃO computo um booleano "dá tempo" (dependeria do status real da pendência) — mostro `diasAtePrazo` + a observação honesta lado a lado.
...
- [x] **T-135 — E-mail diário "Melhor obra pra você hoje" (empurrão proativo)** 🟡
  ...
  - **Feito (2026-07-09) — reusa T-103/T-53, sem dep nova:**
    - **`NotificacoesService.enviarObraDoDia()`:** por usuário notificável (verificado + toggle) **com UF**, reusa **`getEditaisAptos` (T-53)** com o filtro da região (UF + municípios da T-94, `somenteAbertos`), pega o **1º APTO** (não "quase") ainda não enviado (log `obra_do_dia:<editalId>`) e manda **1 e-mail** (template de marca `emailObraDoDia`: objeto, órgão, município/UF, selo "você está apto", valor compacto, botão "Ver o edital"). **Nunca e-mail vazio** (só se houver apto novo); **não repete** a mesma obra.
...
- [x] **T-113 — Spike: pregão + dispensa eletrônica (modalidades PNCP 6/7/8) — medir a lacuna de cobertura** 🔴
...
- [x] **T-114 — Re-sincronizar situação/prazo dos editais abertos (edital morto não é oportunidade)** 🔴
  ...
  - **Dependência:** Épico 2.
  - **PARCIAL (2026-07-06) — a fatia por DATA foi entregue; o núcleo por SITUAÇÃO fica pendente:**
...
- [x] **T-125 — Refinar o classificador de obra + expandir captação (pregão/dispensa) — faseado** 🔴 *(follow-up da T-113)*
  ...
  - **PARCIAL (2026-07-07) — classificador refinado + precisão medida; expansão NÃO ligada (abaixo do limiar):**
    - ✅ **Fase 1 — classificador refinado** (`obra-classifier.ts` + `obra-catalog.ts`): casamento por **limite de palavra** (`\b<stem>`); **strip de padrões negativos** antes de casar positivos ("mao de obra", "materiais/material de construcao"); split **forte / fraca / verbo-de-execução** (fraca só decide com verbo de execução junto ou em modalidade de obra); **hífen/barra → espaço** (senão "mão-de-obra" escapava do strip); `recapeamento` (não `recapea`, que casava "recapagem de pneus"); `engenharia`/`ponte` rebaixados a fracos; `ampliacao`/`implantacao` só como verbos (evita auto-disparo "fraca+verbo do mesmo lema"). +8 testes (armadilhas do T-113). Suíte 383 verde, lint limpo.
    - ✅ **Fase 2 — precisão medida ao vivo** (`spikes/obra-classifier-precision.mjs`, importa o classificador REAL do dist). **Pregão-E (6), SC, 30d:** 2.073 editais → 230 classificados obra (11%); amostra de 45 rotulada à mão → **~16–24% de precisão** (7 obras claras + ~4 limítrofes de engenharia). **Muito abaixo do limiar de 85%** (acordado com o dono). FPs estruturais: "registro de preços para aquisição/fornecimento de" tubos/concreto/materiais + "mão de obra"/"hora-máquina" — dividem vocabulário com obra real; keyword não separa.
    - ✅ **Impacto na Concorrência medido** (old vs new, mod 4/5 SC 30d): **2/612 viram não-obra, ambos corretamente** (locação/compra) — **0 obra real perdida**. O refino é seguro para o produto atual.
    - ⛔ **Fase 3 — NÃO ligada:** `PNCP_MODALIDADES` fica em `[4,5]` (decisão do dono: "ligo 6 só se passar de 85%; senão paro e reporto"). Preset "≤ R$ 80 mil" segue adiado (dependia da dispensa).
    - ➡️ **Caminho para fechar a lacuna (follow-up):** classificação por **intenção de execução via IA** (distinguir "aquisição de materiais para obra" de "execução de obra"), com cache + validação §3.4. É um projeto próprio, não refino de keywords — confirma a conclusão do T-113.
  - **Pronto quando:** a precisão medida numa amostra rotulada fica acima do limiar acordado, e a expansão está ligada **por fase** sem inundar a base de falsos-positivos. *(refino + medição feitos; expansão fica pendente de uma abordagem por IA — precisão de keyword ~20% « 85%.)*

- [ ] **T-140 — Classificar obra por intenção de execução (IA) para destravar pregão/dispensa** 🔴 *(follow-up da T-125; fecha a lacuna medida na T-113)*
  - **Origem:** a T-113 mediu a lacuna (Pregão-E ~100–140 obras reais/mês; Dispensa ~30–60) e a T-125 refinou o classificador de keywords e **mediu a precisão ao vivo: ~16–24%**, muito abaixo do limiar de **85%** acordado com o dono. A fase 3 (ligar `PNCP_MODALIDADES = [4,5,6,8]`) ficou **desligada**. A conclusão das duas tasks é a mesma: **keyword não separa** "aquisição de materiais para obra" de "execução de obra" — elas dividem o vocabulário (`obra`→"mão de obra", `construc`→"materiais de construção", `drenagem`→compra de tubos, `infraestrutura`→TI).
  - **A ideia:** classificar por **intenção de execução** com IA, sobre o `objeto` do edital (texto curto — barato, não é o PDF). Só para as modalidades novas (6/8); a Concorrência (4/5) já tem precisão alta com keyword e **não deve regredir**.
  - **Regras inegociáveis (§3.4):** **cache obrigatório** por edital (é 1 chamada por edital captado, não por busca); **registrar tokens + custo**; **medir o acerto ANTES de ligar** (amostra rotulada, estilo T-125 fase 2 / T-107); rodar na **captação em background**, nunca on-the-fly na busca.
  - **Cuidado de custo (novo em relação às outras tasks de IA):** aqui a IA roda sobre **todo edital captado** das modalidades novas, não sobre os que o usuário abre. A T-113 mediu ~5.600 candidatos/mês em SC nas modalidades 6/8 — a conta de OpenAI escala com a **captação**, não com o uso. Estimar o custo mensal **antes** de ligar, e considerar filtro de keyword como **pré-peneira barata** (só o que passar vai à IA). O teto da T-133 (`IA_BUDGET_*`) vira obrigatório aqui.
  - **Faseado, como a T-125 previa:** medir → **Pregão-Eletrônico (6)** → **Dispensa (8)** com filtro de valor → **pular Pregão-Presencial (7)** (rendimento ~0). Rever o preset "≤ R$ 80 mil" quando a dispensa entrar (hoje é estruturalmente vazio na Concorrência).
  - **Dependência:** T-113 (feito), T-125 (feito até a fase 2), T-118 (resiliência — o volume de 6/8 é muito maior), T-133 (teto de custo).
  - **Pronto quando:** a precisão medida em amostra rotulada passa do limiar acordado, o custo mensal projetado foi aprovado pelo dono, e as modalidades entram **por fase** sem inundar a base de falsos-positivos.
...
- [x] **T-119 — Hardening do cliente e sessão (complementa T-104)** 🟡
  ...
  - **Feito (2026-07-06) — decisões do dono: aprovar `helmet` + refresh em cookie `httpOnly`:**
    - ✅ **(a) Refresh em cookie `httpOnly`:** o refresh token saiu do `localStorage` e virou cookie `httpOnly` (`obrapub_rt`, path `/auth`, `SameSite=None;Secure` em prod / `Lax` em dev por `NODE_ENV`). Backend: `refresh-cookie.ts` (set/clear/parse do header — sem dep `cookie-parser`, tipos do Express à mão como o `UploadedPdf`), `AuthController` lê/seta/limpa o cookie e o corpo devolve só `{accessToken, user}`. Front: `api.ts` usa `credentials:'include'`, `tryRefresh` sem body, `auth.ts` guarda só o access token. +7 testes (controller + parser). **`AuthService` intacto** (a rotação já existia; mudou só o transporte).
    - ✅ **(b) Headers:** `helmet` na API (`main.ts`, CORP `cross-origin`). *(Bloco `headers` do static site fica na T-120 — deploy do front, hoje fora do repo.)*
    - ✅ **(c) Refresh resiliente:** só limpa em **401/403 real** (rede/cold start = status 0 → mantém a sessão); listener de `storage` propaga login/logout entre abas. *(a detecção de rotação por storage saiu com o cookie — o cookie é compartilhado entre abas, o que já reduz a corrida.)*
    - ✅ **(d) `linkOrigem`:** allowlist http(s) no mapper (`sanitizeUrl`) + guarda `httpHref` no front. +2 testes.
    - ✅ **(e) Upload:** validação por **magic bytes** (`detectarMimePorConteudo`) no backend. +1 teste. *(validação de tamanho no front antes de subir: pequeno follow-up, não feito.)*
  - ⚠️ **Falta sign-off ao vivo (§4.4):** o fluxo de **cookie cross-origin** (SameSite=None/Secure em prod, CORS `credentials`, login/refresh/logout no navegador) foi coberto por unit tests mas **não verificado end-to-end** aqui (sem browser/ambiente). **Testar login → refresh (após 15min ou forçado) → logout no navegador, dev e prod, antes de confiar.** Conferir também `WEB_ORIGIN` setado em prod (CORS credentials exige origem específica, não `*`).
  - **Pronto quando:** headers ativos nas duas pontas, refresh resiliente a multi-aba/rede, linkOrigem sanitizado, upload validado por conteúdo. ✅ *(a/b/c/d/e feitos; falta sign-off no navegador e os headers do static site via T-120)*
...
- [x] **T-155 — Access token sai do localStorage e vira cookie httpOnly** 🔴 **(A)** *(pedido do dono, 14/07/2026)*
  - Antes, só o REFRESH (7 dias) era cookie httpOnly; o ACCESS (15 min) vivia no `localStorage` e era anexado à mão no header. Um XSS levava o access — dano limitado, mas real.
...
> - ~~**T-166 — `/perfil` ainda congela (ALTO, NÃO resolvido).**~~ ✅ **RESOLVIDA e fechada em 30/07/2026** (T-166b, em "Tasks soltas"): o dono abriu no navegador e não trava. **Nunca houve loop de render** — quem matou o freeze foi o `keepMounted` da T-178. A parte do **orçamento** (`5cf5824`) também está validada.
> - ~~**Sign-off de UI pendente (§4.4), código pronto:** T-167, T-169 (logout), T-173, T-177, T-178.~~ ✅ **TODOS VALIDADOS no navegador em 30/07/2026 (dono).** O épico não deixa mais residual de evidência.
...
- [x] **T-166 — Congelamento do navegador / loop de render** 🔴 **(A)** — **orçamento CORRIGIDO (`5cf5824`); `/perfil` ainda aberto.**
  ...
  - **Pronto quando:** ✅ **TUDO ATENDIDO (sign-off do dono, 30/07/2026):** BDI negativo mostra o erro sem congelar, `/perfil` abre e reabre estável (T-166b fechada), e existe teste/guarda contra a realimentação (`clampBdi`).

### B — Perda de dados e validação (frustração no primeiro contato)

- [x] **T-167 — Onboarding perde tudo no F5** 🟠 **(B)** — **código pronto e testado (`1b0ed42`); ✅ VALIDADO no navegador (dono, 30/07/2026).**
...
- [x] **T-169 — Deslogado ao voltar do Checkout + falta tela "pagamento confirmado"** 🟢 **(C)** — **código pronto (`51bb84b`); ✅ VALIDADO no navegador, logout incluído (dono, 30/07/2026).**
  ...
  - **Pronto quando:** voltar do Checkout mantém logado e leva a uma tela que comunica o estado real. ✅ **ATENDIDO (sign-off do dono, 30/07/2026).**
...
- [x] **T-173 — Onboarding etapa 2 avança com campos vazios** 🟢 **(C)** — **feito (`d0c66c9`); ✅ VALIDADO no navegador (dono, 30/07/2026).**
  ...
  - **Pronto quando:** o passo 2 (e os demais) só avança com os obrigatórios preenchidos. ✅ **ATENDIDO (sign-off do dono, 30/07/2026).**
...
- [x] **T-175 — Enumeração de conta no cadastro (409 "E-mail já cadastrado")** 🟢 **(C)** — **RISCO ACEITO conscientemente (decisão do dono).**
  ...
  - **✅ Decidido — mantém o 409 (sem código).** O auto-login (T-100) é **incompatível** com não-enumeração: e-mail novo → 201 + logado, e-mail existente → não loga; essa diferença de resposta É o oráculo, então "amenizar a mensagem" não fecha nada. Fechar de verdade exigiria matar o auto-login (todo cadastro viraria "confirme seu e-mail"), desfazendo a T-100. Contexto: B2B, "e-mail existe" é dado de baixo valor, **login e recuperação de senha JÁ são neutros** (`auth.service.ts:107`/`:259`), brute-force barrado (T-104) e o pre-hijacking já fechado (T-159) — o 409 só revela existência, não destrava ataque. **Aceito como risco consciente** (como o CSRF do `SameSite=None`, §8). Ver CLAUDE.md §8.
  - **Pronto quando:** política decidida e refletida (código ou registro de aceite). ✅ (registro de aceite)
...
- [x] **T-177 — Dropdown "Adicionar documento" reabre/rola e desloca os itens** 🟢 **(C — UX)** — **best-effort aplicado (`0ad9308`) e ✅ VERIFICADO no navegador (dono, 30/07/2026) — deixou de ser best-effort.**
...
- [x] **T-178 — Abas de Configurações exigem 2 cliques** 🟢 **(C — UX)** — **best-effort aplicado (`7855b71`) e ✅ VERIFICADO no navegador (dono, 30/07/2026) — e foi ele que matou o freeze da T-166b.**
...
- [x] **T-179 — Publicar textos legais (/termos e /privacidade)** 🟢 **(C — go-live)** — **banner removido (`b9d768b`); texto/placeholders seguem pendentes do dono. ⏸️ ADIADA CONSCIENTEMENTE (decisão do dono, 30/07/2026).**
  - **Por que adiar é a decisão certa, e não procrastinação:** o texto precisa citar a **razão social** e o **processador de pagamento** corretos. Os textos de hoje citam a **Stripe** — e o Épico 17 propõe trocá-la pelo **Asaas**. Escrever agora significaria reescrever depois, **e** disparar um re-aceite de termos (T-196) a mais na cara de todo usuário. **Ordem certa: decidir o Épico 17 → escrever o texto → subir a versão.**
  - ⚠️ **É bloqueador de go-live, não de desenvolvimento.** Nada no código espera por ela; o versionamento da T-196 nasceu **dormente** de propósito e o produto roda normalmente sem versão vigente configurada. **Mas não se lança com placeholder jurídico no ar.**
...
- [x] **T-166b — `/perfil` congela/entra em loop de render** 🔴 — **✅ FECHADA em 30/07/2026: o dono abriu a tela no navegador e ela NÃO trava.** Era o último bug de comportamento aberto do projeto.
  ...
  - **✅ Sign-off (§4.4) FEITO (dono, 30/07/2026): `/perfil` abre e reabre sem travar.** A varredura de 28/07 tinha dito "não há loop no código" — o clique humano **confirmou** o veredito, em vez de contradizê-lo.
  - 📌 **A lição que sobra, e é a mais útil do caso:** o congelamento era **real** no QA de 17/07 e **sumiu sem que ninguém consertasse um loop** — porque loop nunca houve. O que o resolveu foi o `keepMounted` da T-178, que tirou o mount/unmount das abas. **Sintoma de "travou" não implica loop de render**: aqui era remonte pesado sendo percebido como travamento. Antes de caçar `setState` no render, pergunte o que está sendo remontado.
...
- [x] **T-181 — Rota `/admin` no front (lazy, layout próprio, route guard)** 🔴 — **feito (código) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  - **Pronto quando:** chunk separado visível no build; não-admin cai em 404 **idêntico** ao de rota inexistente. ✅ (código)
  - **✅ Feito:** rota `/admin/*` carregada por `lazy(() => import('./pages/admin/AdminArea'))` → **chunk separado no build** (`AdminArea-*.js`, ~2.7 kB gzip) — o **primeiro code-splitting** do app. `AdminRoute` (guard) libera só `role === 'ADMIN'`; o negado (usuário comum OU anônimo) é mandado para `"/"` — **o MESMO destino da rota-coringa `*`**, logo indistinguível de rota inexistente (espelha o 404 do backend, T-180). O front já conhece o `role` (vem de `/users/me`), então não precisou de probe extra para rotear. A decisão foi extraída para `lib/admin-access.ts` (função pura `decidirAcessoAdmin`, com teste — padrão lib/* do projeto). Área com **layout próprio** (`AdminLayout`, distinto do `AppLayout`) + home (`AdminHomePage`) que sonda `GET /admin/me` e prova a cadeia ponta a ponta. 113 testes de front + lint + `tsc` verdes.
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador — a trava está testada em função pura, mas a tela/layout precisam do clique humano.
  - **Dependência:** T-180. ✅

- [x] **T-182 — Audit log de ações admin** 🔴 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador da tela de consulta.
  - **Dependência:** T-180. ✅

- [x] **T-183 — Step-up de autenticação do admin** 🟠 — **feito (reconfirmar senha) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - **✅ Correção (27/07): step-up para admin SÓ-GOOGLE.** O `confirmar` por senha erra fechado para conta sem senha — e o admin do dono nasceu pelo Google, então ele **não conseguia destravar nada** (e o "esqueci a senha" também recusa conta sem senha, `auth.service.ts` — de propósito, T-159). Novo caminho `confirmarComGoogle(userId, idToken)`: reconfirma pela **re-autenticação Google** (popup do SDK → `id_token`), verifica pelo `GoogleVerifierService` e exige `identity.sub === user.googleSub` (mesma checagem da exclusão de conta, T-126). Endpoint `POST /admin/step-up/google` (`@Audit('admin.step-up-google')`); `AdminModule` importa `GoogleAuthModule`. Front: o `StepUpBanner` detecta conta só-Google (`!user.temSenha`) e mostra o `GoogleButton` (modo popup) no lugar do campo de senha — o clique já É a confirmação. Testes: sub próprio destrava; sub alheio → 401; sem googleSub → 400; id_token forjado propaga. Lint+build limpos.
  - **⚠️ 2FA TOTP fica ADIADO** — se a superfície crescer (mais de um admin, dado mais sensível), reabrir.
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (o banner + o fluxo de senha e o de Google).
  - **Dependência:** T-180. ✅

### Visão geral (home)

- [x] **T-194 — Home do admin: números do negócio** 🔴 — **v1 feita (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - **✅ Feito (v2, 24/07 — MRR + custo de IA ligados):** os dois cards que a v1 declarou adiados **já tinham backend pronto** e só não estavam ligados: `GET /admin/billing/mrr` nasceu na **T-192** e `GET /admin/ia-custo` na **T-190b** — inclusive com as funções cliente (`getAdminMrr`, `getAdminIaCusto`) já no `api.ts`. A home passou a agregar os dois: **MRR simples** (com mensais/anuais no subtítulo) e **custo de IA no mês** (com projeção) **e hoje**. Ambos são **best-effort e isolados**: falham em `catch` próprio e viram `—` no card, sem derrubar o painel — o MRR depende da Stripe (o preço vive lá, §8) e não pode levar o resto junto. Formatadores **reusados** (`brlDeCentavos` do `AdminBillingPage`, `usd` do `CustoIaSection`, ambos apenas exportados) — sem terceira cópia.
  - **⚠️ NÃO existe card de "margem"** e é decisão, não esquecimento: a receita é **BRL** (Stripe) e o custo de IA é **USD** (OpenAI), e não há câmbio no sistema. Subtrair um do outro produziria um número errado com cara de certo — o tipo de tela que mente pro dono (Épico 10). A ressalva está escrita na própria tela. Se a margem for necessária, ela exige uma decisão de câmbio primeiro.
  - **⚠️ Segue adiado:** **funil de ativação + conversão por coorte** (fase seguinte) e "cancelamentos **no mês**" (não há `canceled_at`; a tela mostra o total, não o mês). "Hoje" conta desde o início do dia **UTC** (ressalva na tela).
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador.
  - **Dependência:** T-180, T-181. ✅

### Contas e operação do beta

- [x] **T-184 — Lista e detalhe de contas** 🔴 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - **✅ Resolvido (24/07): os contadores de IA por conta ENTRARAM** — a ressalva abaixo caiu com a T-190a. O detalhe ganhou a seção **"Uso de IA"** (exigências+resumo, itens da planilha, chamadas à OpenAI, servidas do cache, custo atribuído em USD), lida do `ai_usage`. ⚠️ **Conta só a partir de 24/07/2026** (a tela avisa): antes disso o custo era por EDITAL e não havia como dizer de quem era — não há retroativo.
  - **⚠️ Ressalva de escopo ORIGINAL (superada, mantida como registro):** resumos IA e diagnósticos foram adiados na entrega da T-184 porque não eram atribuíveis por conta (cache de IA é por edital; diagnóstico é computado na hora). Os contadores da primeira leva (favoritos, propostas, alertas enviados, certidões, atestados) sempre foram por-usuário.
  - **✅ Correções do 1º sign-off (27/07 — feedback de teste do dono), abrangem T-184/185/188/199/183:**
    - **Status "Em teste" mentia num trial vencido, e a cortesia não aparecia.** A raiz era a mesma: a tela mostrava o **status cru do banco** (trial vencido segue `trialing`; cortesia/suspensão não mexem no status). Agora o backend calcula o **acesso REAL** (`calcularAcesso`, §3.3) e devolve um `AcessoResumo`; a tela mostra um **selo de situação honesto** — `Ativa` · `Em teste · faltam Xd` · **`Trial expirado · sem acesso`** · **`Cortesia ativa`** · `Suspensa · sem acesso` · `Pagamento pendente` — na lista E no detalhe (o status cru da Stripe fica como campo secundário).
    - **Verificar conta com 1 clique (T-185):** nova ação `POST /admin/accounts/:id/verificar-email` (auditada `account.verify-email`, step-up) que marca `email_verified_at` na hora (idempotente); botão "Verificar agora" no card de Ações (desabilitado se já verificado), com confirmação.
    - **"Rodar notificações" parecia quebrado (T-188):** o endpoint era fire-and-forget e engolia o resultado — 0 elegíveis (sem e-mail verificado/toggle/obra apta) era indistinguível de erro. Agora é **síncrono e devolve as contagens** (`{ alertas, obrasDoDia, renovacoes }`); a tela mostra "enviados: X/Y/Z" ou "0 — ninguém elegível agora". Também rótulos explicando o que o disparo faz.
    - **Rodar captação com escolha de UF (T-188):** `runOnce(ufs?)` aceita UFs escolhidas (senão orientada à demanda, T-34); DTO `RodarCaptacaoDto`; a tela ganhou um multiselect de UF (vazio = todas com usuário ativo) e explica o alvo do disparo.
    - **Buscas vazias com município (T-199):** a lista de buscas sem resultado passou a resolver os **códigos IBGE → "Nome/UF"** (repo `Municipio` no admin) e mostrar a coluna Município(s).
    - **Step-up POR SESSÃO (T-183, decisão do dono):** o "modo sudo" deixou de reautenticar a cada 10 min — desbloqueia **uma vez e vale até deslogar** (janela de 7 dias como teto; `AuthService.logout` limpa `admin_stepup_ate` → o próximo login recomeça travado). 872→ (+testes) API verdes, front verde, lint+build limpos.
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador de lista e detalhe.
  - **Dependência:** T-180, T-181. ✅

- [x] **T-185 — Ações de conta** 🔴 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador das ações.
  - **Dependência:** T-182, T-184. ✅

- [x] **T-186 — Notas internas por conta** 🟢 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-187 — Impersonation ("ver como") com salvaguardas** 🟢 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (banner, "ver como", bloqueio de mutação, sair).
  - **Dependência:** T-182, T-184. ✅

- [x] **T-196 — Operação LGPD e aceite de termos** 🟠 — **v1 (fila) + versionamento de termos feitos (backend + front) e VALIDADOS no navegador (sign-off do dono, 30/07/2026). Só falta o dono "ligar" (T-179).**
  ...
  - **✅ Feito (27/07 — o versionamento + re-aceite, a parte que estava adiada):** a mecânica nasce **DORMENTE** e text-agnóstica — **nada de texto jurídico inventado** (isso é a T-179, entrega do dono). Enquanto não houver **versão vigente** configurada, ninguém é forçado a re-aceitar (comportamento idêntico ao de hoje); quando o dono publicar o texto e **subir a versão**, o re-aceite passa a valer sozinho.
    - **Versão vigente** vive no config store (T-195) — nova chave `terms_version` (string curta, ex.: `2026-07-27`), editável em `POST /admin/config/terms-version` (auditado `config.terms-version`, **sem step-up** como as demais config). Vazia = versionamento desligado. Card "Versão dos termos" na `AdminConfigPage`, com o aviso de só subir **depois** de publicar `/termos` e `/privacidade`.
    - **Qual versão a conta aceitou:** nova coluna `users.terms_version` (migration `AddTermsVersionToUsers`, nullable — contas antigas nascem sem versão). Cadastro (e signup Google) **carimba a versão vigente** no aceite — senão um recém-cadastrado já cairia no re-aceite.
    - **A decisão** é a função pura `precisaReaceitarTermos(aceita, vigente)`: só força quando há vigente E é diferente da aceita; sem vigente → sempre false. Exposta no `/users/me` como `precisaReaceitarTermos` (e já no login/sessão Google — conta antiga pode estar desatualizada e o front guarda esse user antes de um /me).
    - **O aceite:** `POST /users/me/aceitar-termos` (JWT, **FORA do paywall** — um usuário bloqueado precisa poder re-aceitar) grava a versão **VIGENTE DO SERVIDOR** (não confia numa versão vinda do cliente) + o instante. Front: `ReaceiteTermosGate` na cadeia de portões do `AppLayout` (após verificação de e-mail, antes do paywall) — "Atualizamos nossos termos" + links para `/termos` e `/privacidade` + "Li e concordo". **Texto genérico, sem conteúdo jurídico** (o conteúdo é do dono, T-179).
    - **Admin:** o detalhe da conta passa a mostrar a **versão** aceita ao lado da data (`Aceite dos termos: 27/07/2026 (v. 1.0)`).
    - **Testes:** `precisaReaceitarTermos` (sem vigente/igual/diferente) + `UsersService.aceitarTermos` (carimba versão+data; 404) + config store (`getTermsVersion` null/trim/vazia→null; `setTermsVersion`). Lint+build limpos.
  - **⚠️ Segue com o dono (T-179):** publicar o **texto jurídico definitivo** em `/termos` e `/privacidade` e então **setar a versão** no admin — só aí o re-aceite dispara. **Fora de escopo (consciente):** histórico de TODAS as versões aceitas por conta (guardamos a última; a fila LGPD é a prova documental); botão "solicitar" in-app novo (o self-service T-102 já cobre o titular logado); e **enforcement no backend** do re-aceite (hoje o portão é do front, no espírito §3.3 o consentimento é UX de consentimento, não fronteira de acesso como o paywall — se precisar barrar a API, vira task).
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (a fila LGPD; e o portão de re-aceite — que só aparece com uma versão configurada no admin).
  - **Dependência:** T-184 ✅. T-179 (só o "ligar": publicar o texto + setar a versão).

- [x] **T-202 — Fila de feedback/bug do usuário (reporte in-app)** 🟠 *(no beta; 🟢 fora dele)* — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (o botão de reporte + a fila).
  - **Dependência:** T-181 (front), T-182 (registro). ✅

### Pipeline PNCP e IA

- [x] **T-188 — Painel de captação e jobs** 🔴 — **v1 feita (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-190 — Medidor de custo de IA** 🟠 — **T-190b (a tela) e T-190a (instrumentação `ai_usage`) FEITAS; falta a LEITURA por conta / hit rate.**
  ...
  - **✅ Feito (T-190a — a instrumentação, 24/07):** tabela **`ai_usage`** (append-only, uma linha por chamada real **ou** por cache hit) + `AiUsageService` (só o **write**). Campos: `feature` (`exigencias` cobre exigências E resumo — saem da MESMA chamada, separá-las inventaria custo que não existe), `origem` (`usuario`/`precomputacao`/`admin`), `user_id`, `edital_id`, `cache_hit`, `modelo`, tokens e `custo_usd`. **Sem FK** em `user_id` nem `edital_id`: é registro contábil e precisa sobreviver à exclusão da conta (LGPD) e à retenção de editais (T-154) — mesma razão do `admin_audit_log`.
    - **Como o usuário chega lá:** `getOrExtract(editalId, ctx?)` ganhou um contexto opcional (`{ origem, userId }`), passado explicitamente nos **5 call sites** (detalhe do edital, itens da planilha, diagnóstico do perfil, import da proposta, curadoria do admin) e omitido só na pré-computação, cujo default é `precomputacao` sem usuário. Explícito, sem AsyncLocalStorage — o contexto **não muda extração nem cache**, só o registro.
    - **Custo ZERO obrigatório no cache hit** (o service zera mesmo se o chamador mandar valor): o custo do cache já foi pago uma vez e está na linha de cache; contá-lo de novo a cada leitura inflaria o gasto por conta.
    - **A chamada de itens é registrada ANTES de ramificar pelo resultado:** a OpenAI foi paga mesmo quando o desfecho é "indisponível" (leu o texto e não era planilha). Registrar só o caminho feliz subestimaria o custo justamente no caso que mais desperdiça.
    - **Best-effort:** `registrarEmSegundoPlano` no padrão do `SearchLogService` (T-199) — erro morre em log + Sentry e **nunca** derruba a extração. Testado: rejeição do banco não vira `unhandledRejection`.
    - ⚠️ **NÃO é a fonte do teto (T-133) nem da tela (T-190b):** as duas continuam somando as tabelas de cache. `ai_usage` **nasce vazia**, e migrar o circuit-breaker para ela **zeraria o gasto acumulado, desligando o teto no caminho do dinheiro**. Convivência deliberada: totais pelo caminho antigo, recortes novos por aqui. 827 testes de API verdes.
  - **✅ Feito (a leitura, 24/07):** `AdminAiUsageService` (read no admin, espelhando a separação write/read do `SearchLogService`/`AdminSearchLogService` da T-199) com `hitRate`, `porConta` e `daConta`, tudo por `COUNT(*) FILTER (WHERE …)`. `GET /admin/ia-custo` ganhou `hitRateMes`, `porContaMes` e **`inicioHistorico`**. Na tela: card de **hit rate** + tabela **custo por conta** (link para o detalhe), e o detalhe da conta ganhou a seção **"Uso de IA"** (exigências+resumo, itens, chamadas, servidas do cache, custo atribuído) — o que a **T-184** tinha deixado pendente.
    - **`taxa` é `null`, não 0, quando não houve acesso nenhum** — "o cache nunca serviu" e "ainda não medimos nada" são coisas diferentes, e com o `ai_usage` recém-criado a segunda é a verdade. A tela mostra `—`.
    - **`porConta` EXCLUI o uso sem dono** (pré-computação em background): não é de ninguém, e atribuí-lo a alguém seria mentira. Consequência assumida e **escrita na tela**: a soma da lista é MENOR que o gasto do mês.
    - **Conta excluída não some da lista:** sem FK (registro contábil), o `user_id` pode não ter mais dono — a linha aparece como "(conta excluída)" em vez de desaparecer com o custo.
    - **`inicioHistorico` existe para a tela não parecer quebrada:** sem ele, um painel zerado no dia seguinte ao deploy se lê como defeito, e não como "ainda não houve uso desde que passamos a medir".
    - **A tela declara os DOIS históricos**: totais/projeção/por-dia vêm das tabelas de cache (completo); hit rate e por-conta, do `ai_usage` (desde 24/07). Misturá-los num número só faria o painel mentir sobre o período coberto. 833 API + 125 front verdes.
  - **✅ Feito (o alerta de teto por e-mail, 27/07):** a T-190 fecha. O circuit-breaker (T-133) já BLOQUEAVA no teto, mas em silêncio — agora o dono é **avisado por e-mail**, com **dois níveis (decisão do dono): aviso prévio em ~80%** (`LIMIAR_AVISO_TETO`, dá margem para subir o teto antes de a IA parar) **e "atingido" em 100%** (IA já pausada). Novo `IaCustoService.alertasDeTeto(now)` (puro, `now` injetável): para cada período COM teto (diário/mensal), `pct = gasto/teto` → um nível por período (o mais alto, nunca 'aviso' E 'atingido' juntos); **sem teto configurado → vazio** (comportamento padrão intacto).
    - **Reusa TODA a maquinaria da T-189** (`PipelineHealthAlertService`): cooldown por tipo na `pipeline_alert_state` (**sem migration** — a tabela é genérica `(tipo, last_sent_at)`), `ADMIN_ALERT_EMAIL`, `@Cron` 6h **+** o endpoint de ops `POST /captacao/verificar-saude` (o cron externo já bate nele). O `IaCustoService` já é exportado pelo `EditaisModule`, que o `CaptacaoModule` importa → **sem mexer em fiação**.
...
- [x] **T-191 — Fila de revisão do classificador** 🟢 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-199 — Log de busca sem resultado + o que estão buscando** 🟠 *(o sinal mais rico de um produto de captação)* — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-200 — Amostra de saídas de IA para conferência ("a IA acertou?")** 🟠 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-201 — Painel de saúde das integrações + sanidade de env (anti-T-163)** 🟠 — **v1 feita (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-192 — Espelho de assinaturas + log de webhooks Stripe** 🟠 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
...
- [x] **T-193 — Log de e-mails transacionais** 🟠 — **feito (nível de ENVIO); entrega/bounce e reenvio genérico adiados. Sign-off de UI pendente.**
  ...
  - **✅ Feito (o status de ENTREGA, 27/07 — a v1 estava adiada, agora fechou):** **webhook do Resend** `POST /webhooks/resend`, público e autenticado pela **assinatura Svix sobre o corpo CRU** (o Resend usa Svix; verificação HMAC-SHA256 à mão em `resend-signature.ts` — **sem SDK/dep nova**, mesmo espírito do webhook da Stripe e `rawBody: true` no `main.ts`). `ResendWebhookService` casa o evento com a linha de envio pelo **`provider_message_id`** — novo campo em `mail_log`, preenchido com o `id` que o Resend devolve no POST de envio (o `enviarPorHttp` agora lê o `{ id }` da resposta; falha ao ler o id **nunca** marca o envio como falho — try próprio). Tipos mapeados: `email.delivered`→entregue, `email.bounced`→bounce, `email.complained`→reclamacao, `email.delivery_delayed`→atrasado; os demais, ignorados.
    - **Guarda de ordem (eventos fora de ordem, como na Stripe):** um `delivered` que chega **depois** de um `bounce` NÃO sobrescreve o bounce (negativo definitivo vence; `RANK` por status). Mesmo status = no-op → **idempotência** para reentrega.
    - **`RESEND_WEBHOOK_SECRET` ausente → webhook desabilitado** (responde 200 + loga, para o Resend não reentregar eternamente; mesma degradação dos outros provedores). Assinatura inválida → 200 e descarta (um 4xx só faria reentregar algo que seguiria inválido). Processar o evento é best-effort (erro → Sentry, nunca derruba a resposta).
    - **Migration `AddMailDelivery`:** `provider_message_id` (+índice), `delivery_status`, `delivery_at`, `delivery_detalhe` em `mail_log`. Front: coluna **"Entrega"** na `AdminMailLogPage` (badge entregue/bounce/reclamação/atrasado; `—` sem sinal; detalhe do bounce no title). Envs no CLAUDE.md §8 + nota do T-163.
    - **Testes:** assinatura Svix (válida/adulterada/segredo errado/header ausente/sem segredo/replay por timestamp/múltiplas assinaturas) + `ResendWebhookService` (delivered/bounce gravam; bounce não é sobrescrito por delivered posterior; duplicado = no-op; tipo e id desconhecidos; sem email_id) + `MailService` captura o id do Resend no log. **859 API + 125 front verdes.**
  - **⚠️ Segue fora de escopo:** **reenvio manual genérico** (não dá para "reenviar" um e-mail transacional arbitrário do passado de forma significativa) — o **reenvio de verificação** já existe como ação de conta na **T-185**. Filtro por status de ENTREGA na lista do admin também fica para quando doer (a coluna aparece; o filtro atual é por status de ENVIO).
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (a coluna de entrega ao vivo depende de `RESEND_WEBHOOK_SECRET` configurado + evento real do Resend).
  - **Dependência:** T-184. ✅

- [x] **T-198 — Comunicado ao beta** 🟢 — **feito (backend + front) — **VALIDADO no navegador (sign-off do dono, 30/07/2026).**
  ...
  - ✅ **Sign-off (§4.4) FEITO (dono, 30/07/2026)** — validado no navegador (compor, preview, enviar com step-up, histórico).
  - **Dependência:** T-184. ✅
...
## Épico 16 — Camada de borda Cloudflare (29/07/2026) — ✅ FECHADO em 30/07/2026

> **Placar final:** **T-203** (Turnstile) ✅ · **T-204** (proxy + IP real + Full strict) ✅ · **T-205** (Access no `/admin`) ✅ · **T-206** (rate limiting na borda) ❌ **descartada**. Dois dias do começo ao fim.
>
> **As três lições que este épico deixa, e que valem mais que o código dele:**
> 1. **A premissa de duas das quatro tasks estava errada — e nos dois casos a favor do trabalho desnecessário.** A T-204 mandava "ligar o proxy" que **já estava ligado**; a T-206 mandava consertar um storage cujo furo, medido, é de **uma janela de 60s**. Em ambos, o que revelou o erro foi **medir antes de codar**. ⚠️ Antes de executar task de infra escrita há mais de uma semana, **confira a premissa dela contra a realidade**.
> 2. **"Todos os pontos que leem X" inclui o default das bibliotecas.** O primeiro commit da T-204 corrigiu os três pontos que um `grep req.ips` encontra e passou pelo mais importante, que morava no `node_modules`.
> 3. **Escolher a métrica errada esconde o problema.** O aceite pedia "p99 dos endpoints de IA"; o p99 descreveria o caso comum (cache hit) e não mostraria que o **teto** do caminho síncrono é de 8+ min contra 100s da borda. Quando existe teto estrutural, meça o teto, não o percentil.
>
...
2. **`@nestjs/throttler` já está no projeto e o login já é limitado nas duas dimensões** (T-104): 5/min por IP + 5/min por e-mail (`EmailThrottlerGuard`) — mais apertado que os 10/10min propostos na borda. O "plano B" da T-206 **não é implementar rate limit**. ~~O buraco real é o **storage em memória**~~ 🔴 **CORRIGIDO em 30/07:** essa afirmação era grande demais. Com janelas de **60s** e **zero `blockDuration`**, um reinício custa ao atacante uma janela (5 tentativas) — e a hibernação, que é por **inatividade**, não dispara durante um ataque em curso. O que resta de verdadeiro é **instâncias não somarem**, que é escala horizontal. **A T-206 foi descartada por isso** (decisão do dono, 30/07) — detalhe na task.
...
- [x] **T-204 — Ligar o proxy Cloudflare (nuvem laranja)** 🔴 (infra) — **✅ FECHADA em 30/07/2026.** Código entregue, medições de aceite feitas e **`SSL/TLS` virado para Full (strict) pelo dono**. Fumaça depois da troca: `api /health` **200**, `app /` **200** — um strict com origem inválida apareceria como **525/526**, não como 200.
  ...
  - ✅ **Webhooks e rotas públicas reconferidos com o proxy na frente (30/07), por medição:** `/health` **200** (`server: cloudflare` + `cf-ray` + `x-render-origin-server: Render`), webhook da Stripe **400 do Nest** (assinatura inválida — ou seja, o corpo chega na aplicação), webhook do Resend **200**, `/auth/login` **400**. O app do Access escopado a `api…/admin` (T-205) não pega nada do produto, como previsto.
  - ✅ **`SSL/TLS` Full (strict) é SEGURO — e a evidência NÃO depende do painel (30/07).** O teste que decide **não** é o certificado do `*.onrender.com`: a Cloudflare conecta na origem com o **SNI do hostname de produção**. Resolvendo o IP da origem (`216.24.57.15`, o mesmo para os dois serviços) e abrindo TLS com esse SNI, o Render serve certificado válido e publicamente confiável **para o nome de produção**, não um cert genérico:
    - `obrapub-api.onrender.com` + SNI `api.prumolicita.com.br` → `CN = api.prumolicita.com.br`, Google Trust Services, até **11/10/2026**
    - `sass-obra-pub.onrender.com` + SNI `app.prumolicita.com.br` → `CN = app.prumolicita.com.br`, Google Trust Services, até **11/10/2026**
    - **Reproduza com:** `IP=$(getent hosts obrapub-api.onrender.com | awk '{print $1}' | head -1); echo | openssl s_client -connect $IP:443 -servername api.prumolicita.com.br 2>/dev/null | openssl x509 -noout -subject -dates`
    - ⚠️ **O modo atual já é Full ou Full (strict), não Flexible** — dá para afirmar de fora: em Flexible a Cloudflare falaria HTTP com a origem, o Render redirigiria para HTTPS e o resultado seria **loop de redirecionamento**. Como o site responde 200, Flexible está descartado. O que **não** dá para distinguir de fora é Full de Full (strict) — daí o clique.
    - 🔴 **Falta só isso na task:** virar o seletor no painel. O token OAuth do MCP não lê **nem escreve** configuração de zona (ver também a memória "MCP da Cloudflare: escopo limitado").
  - ✅ **Cache de borda confirmado funcionando:** asset com hash (`/assets/index-*.js`) responde `cf-cache-status: REVALIDATED` na 1ª e **`HIT`** na 2ª; o HTML da SPA vem `DYNAMIC` (**não** cacheado), que é exatamente o que esta task pedia para não quebrar deploy novo.
    - 📌 **Ganho disponível, NÃO aplicado (decisão do dono, 30/07: "não agora"):** o Render manda `cache-control: public, max-age=14400, s-maxage=300` — **`s-maxage` de 5 min num arquivo com hash no nome, que é imutável por construção**. A borda revalida de graça a cada 5 min. Uma *Cache Rule* em `/assets/*` com Edge TTL longo faria o cache de borda pagar de verdade. Clique de painel, sem código.
  - 🔴 **O item "medir o p99 dos endpoints de IA" foi RESPONDIDO SEM p99 — e a resposta é pior que um percentil (30/07).** Não é preciso estatística para saber que 100s é excedível: **o caminho síncrono não tem orçamento total nenhum.** `GET /editais/:id/exigencias` e `GET /editais/:id/itens-extraidos` chamam `getOrExtract` **dentro da requisição**, e os tetos de cada etapa somam:
    - listagem de arquivos do PNCP: `PNCP_TIMEOUT_MS` 20s × `PNCP_MAX_ATTEMPTS` 6 → até **120s** só aí, mais backoff (`pncp.constants.ts`)
    - loop de candidatos: `rankPncpArquivos` devolve **todos** os arquivos ativos, **sem teto de N**, e cada um tem `DOWNLOAD_TIMEOUT_MS` de **60s** + parse de PDF em CPU (`exigencias.service.ts`, passo 2)
    - OpenAI: `OPENAI_TIMEOUT_MS` 120s × `maxRetries: 2` → até **360s** (`ia-extracao.service.ts:17`)
    - **Teto somado: mais de 8 minutos, contra os 100s da borda.** E a hibernação do Render entra por cima: medido em 30/07, o serviço dormindo não respondeu nada em 30s; a chamada seguinte levou **12,8s**; quente ele fica em 0,4-0,8s.
  - ✅ **DECISÃO REGISTRADA (dono, 30/07): manter o `api` proxiado, e o caminho >100s fica como RISCO ACEITO, sem código.** Três mitigantes já existentes, todos verificados no código, é o que sustenta isso — **não é otimismo**:
    - a **T-54 pré-computa** os editais das UFs ativas no job de captação, então requisição de usuário costuma ser cache **hit** (rápida);
    - o **`inFlight` deduplica por `editalId`**: cliente que toma 524 e recarrega se **anexa** à tarefa em andamento — **não dispara segunda chamada de IA**, logo o timeout da borda **não dobra custo**;
    - o resultado **persiste mesmo com o cliente desconectado** (o Node não aborta a tarefa quando a conexão cai).
    - **Sintoma real, portanto:** 1ª visita a edital ainda não pré-computado pode morrer em 100s com erro na tela; um refresh ~1 min depois mostra o resultado pronto. Degradado e **auto-curável**, não perda de dado nem custo dobrado. ⚠️ O retry do `lib/api.ts` (T-205) é escopado a `/admin` **de propósito** e **não** cobre isso.
    - **O plano B (voltar `api` para DNS-only) foi DESCARTADO:** mataria a Etapa 2 da T-205, que já está no ar protegendo os dados do `/admin`. Preço alto para um caminho raro e auto-curável.
    - **Gatilho para reabrir:** se o beta mostrar esse erro na mão de cliente real, as opções avaliadas foram (a) apertar nossos tetos para caber em 100s — barato, mas degrada a extração de planilha grande, que é justamente por que os 2 min existem (T-64); (b) **assíncrono + polling (202 "processando")**, que é o desenho certo e some com o 524 de vez, mas muda contrato de API + duas telas (1-2 dias) e **merece número de task próprio, não cabe aqui**.
  ...
  - **Pronto quando:** ✅ **TODOS OS ITENS ATENDIDOS.** ~~SSL/TLS em Full (strict)~~ ✅ (dono, 30/07 — a segurança de virar estava provada pelo teste de certificado com SNI de produção, e a fumaça pós-troca deu 200 nos dois hostnames); ~~`app` proxiado~~ ✅ (já estava, desde 28/07); ~~auditoria de leitura de IP concluída e `CF-Connecting-IP` num ponto só~~ ✅ (`07fbf6f`+`7c37768`); ~~log de auditoria do `/admin` com IP real~~ ✅ (mesma função do guard medido); ~~decisão registrada sobre o `api` (proxiado ou plano B) **com o número que a embasou**~~ ✅ (30/07 — manter proxiado; o número é o teto de **8+ min** do caminho síncrono, não um p99: ver acima **por que o percentil era a pergunta errada**); ~~webhooks da Stripe e do Resend confirmados~~ ✅ (medidos em 30/07).

- [x] **T-205 — Cloudflare Access na frente do `/admin`** 🟠 (segurança) — **FEITO em 29/07/2026, AS DUAS ETAPAS. Nenhuma linha de código.**
  - **⚠️ NADA DISTO ESTÁ VERSIONADO.** É configuração de conta Cloudflare, criada via API — não há `git revert`. Se a conta for perdida ou alguém apagar os apps, o registro abaixo é a única fonte para recriar:
    - **Team domain:** `prumolicita.cloudflareaccess.com` (org "PrumoLicita", `session_duration: 8h`)
    - **IdP:** One-time PIN, id `1551aa52-b7dc-4daf-b857-7f4ca4a5fa97` ("Código por e-mail")
    - **App documento:** id `c305d963-68cd-4a00-91a7-d4a857b17479`, `domain: app.prumolicita.com.br/admin`
    - **App dados:** id `b6f9bcbd-9a27-47a5-8a6a-dcc6930b1bac`, `domain: api.prumolicita.com.br/admin`, `options_preflight_bypass: true`, `same_site_cookie_attribute: lax`
    - **Política nos dois:** `allow` → `include: [{ email: { email: "ps6711534@gmail.com" } }]`; `allowed_idps: []` (todos)
  - 🔴 **DUAS LISTAS DE PERMISSÃO AGORA, em lugares diferentes — é a pegadinha operacional desta task.** O Access **não sabe nada** sobre a role `ADMIN`: não lê o JWT, não consulta o banco. Ele decide por **caminho + e-mail**. Consequência: **tornar alguém admin no banco NÃO dá acesso** — o e-mail dessa pessoa também precisa entrar na política do Access. O sintoma será "é admin e não abre o `/admin`", e a causa está na Cloudflare, não no código.
  - **O que cada camada cobre (nenhuma substitui a outra):** o Access barra quem **pede a URL** (entrada direta, reload, e as chamadas de dados); o `AdminGuard` (T-180, 404 para não-admin) barra quem **já está dentro da SPA** e navega por client-side routing — caso em que não há requisição de documento e o Access nem é consultado. A limitação que o rascunho da task previa **se confirmou**, e é por isso que proteger só o documento seria quase teatro: os dados vêm de outro hostname.
  - ✅ **Verificado por medição, o que NÃO quebrou** (o risco real da Etapa 2): webhook da Stripe segue em `400` do Nest, webhook do Resend em `200`, `/health` em `200`, `/auth/login` em `401`. **O escopo TEM de ser `api…/admin`, não o hostname inteiro** — o hostname todo barraria os dois webhooks (públicos, sem identidade) e a API do produto inteira, com o pior sintoma possível: cliente que pagou preso no paywall, em silêncio.
  - ✅ **Preflight CORS:** `OPTIONS /admin/contas` responde `204` com `allow-credentials: true` e `allow-origin: https://app.prumolicita.com.br`. ⚠️ A API do Access **PROÍBE** combinar `options_preflight_bypass` com `cors_headers` (`12130 invalid_request`) — ou a borda cuida do CORS, ou a origem. Ficou na origem, porque o Nest já faz `enableCors` com credenciais e headers duplicados seriam pior.
  - ✅ **Sessão entre os dois apps:** um código só. Autenticar no `app` cobre o `api` sem novo PIN.
  - 🔴 **O cookie é POR HOSTNAME, e isso quebrava a primeira carga do painel** (observado pelo dono em produção; **corrigido em `1714d7a`**). Autenticar em `app…/admin` dá cookie para o `app`, **não** para o `api`. Então a 1ª chamada de dados numa sessão nova (a cada 8h) toma 302 para `cloudflareaccess.com` — outra origem, sem headers de CORS — o `fetch` falha, e o painel **abria quebrado, exigindo refresh manual**. O `Set-Cookie` do caminho É honrado pelo navegador: **a tentativa que falha é a que cria o cookie**, e a seguinte passa.
    - **Correção:** o cliente de API repete **uma vez** em falha de rede, **só em rotas `/admin`** (`lib/api.ts`). Faz o que o refresh manual fazia. Construída sobre o único fato medido (a 2ª tentativa passa), não sobre o mecanismo, que é inferência. Não repete em erro HTTP nem em rota do produto — 4 casos em teste.
    - ⚠️ **Para validar isso no navegador é preciso uma sessão NOVA do Access** (fechar TODAS as janelas anônimas, ou `https://prumolicita.cloudflareaccess.com/cdn-cgi/access/logout`). Com o cookie do `api` já emitido, o caminho da falha não acontece e o teste dá falso positivo.
    - 📌 **Alternativa não testada:** unificar os dois apps num só via `destinations` (um app cobrindo os dois caminhos). Deixaria a config mais limpa, mas **não se sabe se resolve a 1ª carga** — o cookie segue sendo por hostname. Só valeria investigar se o retry se mostrar insuficiente.
  - ⚠️ **Cuidado ao interpretar teste em janela anônima:** ela compartilha cookies entre TODAS as abas e janelas, e só limpa quando a última fecha. Isso produziu um falso negativo aqui ("o Access não aparece quando estou deslogado") que na verdade era o `CF_Authorization` da autenticação anterior. **A sessão do Access é por NAVEGADOR/dispositivo — o dono estar logado não abre porta para mais ninguém** (verificado: requisição de outra máquina, sem cookie, recebe 302 enquanto o dono navega normalmente).
  - ⚠️ **Achado que contraria a doc lida antes:** **One-time PIN NÃO é default.** Com `access/identity_providers` vazio, o Access mostra *"There are no login methods available for this account"* e ninguém entra — nem o dono. Criar o IdP `onetimepin` é passo obrigatório, e lista vazia significa **ausência de método**, não presença de default. Foi o que aconteceu aqui: os apps foram criados antes do IdP.
  - **Escopo original desta task, para referência:**
...
- [x] **T-206 — Rate limiting no login (borda)** 🟢 (segurança) — **DESCARTADA em 30/07/2026 (decisão do dono). Não recrie esta task sem ler os três parágrafos abaixo.**
  - 🔴 **A premissa desta task — "o storage em memória é o buraco que vale mais" — NÃO se sustenta quando medida contra a configuração real.** Duas conferências no código derrubam o tamanho do problema:
    1. **Todas as janelas são de 60 segundos** (`JANELA_MS = 60_000`, `throttle.config.ts`) e **não existe `blockDuration` em lugar nenhum** (verificado por grep em todo o `src`). O estado que um reinício destrói é um contador que ia expirar sozinho em ≤1 min. O ganho do atacante por reinício é **uma janela extra** — 5 tentativas no login. Não é o que decide um brute-force.
    2. **A hibernação não acontece durante um ataque.** O Render hiberna por **inatividade** (~15 min); um atacante martelando o login mantém o serviço acordado — **o próprio ataque impede o reinício que zeraria o contador**. O cenário descrito exige um atacante que ataca, para por 15 min e volta: esse já estava respeitando o limite.
  - **O que sobra e é verdade:** num servidor que não hiberna, o reset passa a vir do **deploy** (contínuo no push para `main`) — mesmo ganho de 5 tentativas, poucas vezes por dia. E **múltiplas instâncias não somam contadores** (2 instâncias = limite efetivo dobrado) — esse é o único motivo real para storage compartilhado, é **escala horizontal, não hibernação**, e hoje é uma instância só.
  - **Decisão do dono (30/07):** vai trocar de servidor antes do lançamento, então o argumento da hibernação morre; a task inteira foi descartada, **incluindo a regra de borda**, que ia junto. A regra nunca foi proteção nova — o login já tem 5/min por IP **e** 5/min por e-mail (T-104), ambos mais apertados que os 10/10min propostos; ela só pouparia CPU do Nest e ciclo de bcrypt. **Ressuscitar é clique de painel de 5 min**, se um dia o custo aparecer.
  - ⚠️ **Se um dia quiser brute-force de verdade, o item NÃO é o storage — é bloqueio progressivo** (`blockDuration`: após N falhas, a conta fica barrada por X min). Aí sim existe estado longo que vale persistir, e o storage vira **consequência, não causa**. É mudança de comportamento, merece task própria, e tem custo real: barrar conta por tempo é vetor de negação de serviço contra usuário legítimo (basta errar a senha dele de propósito).
  - ⚠️ **Gatilho para reabrir:** escalar para **mais de uma instância** da API. Aí os contadores em memória deixam de somar e o limite efetivo vira N × limite — esse furo é independente de hibernação e de deploy.
  - 📌 **A regra de borda, se voltar a ser desejada, é painel:** o token do MCP **lista** rulesets da zona mas **não é autorizado** no phase `http_ratelimit` (medido em 30/07) — nem leitura. Expressão que estava pronta: `hostname eq "api.prumolicita.com.br" and starts_with(http.request.uri.path, "/auth/login")`, 10 req/IP em 10 min, ação **managed challenge** (não *block*: construtora com vários usuários atrás do mesmo IP corporativo é exatamente o público-alvo).
...
- [x] **T-207 — Spike: modelo de checkout e exposição PCI** 🔴 (bloqueadora) — **FEITA em 30/07/2026. Resultado completo em `spikes/RESULTADOS.md` (seção T-207).** ⚠️ Spike de **leitura de documentação**, não de código — sem conta Asaas ainda (T-209); o que está marcado 🔬 lá precisa de confirmação no sandbox.
  - ✅ **DECISÃO: opção A — Checkout HOSPEDADO (`POST /v3/checkouts`) para tudo, inclusive troca de cartão. O escopo PCI continua SAQ A, igual ao de hoje.** O cliente digita o cartão na página do Asaas; recorrência de verdade (`chargeTypes: RECURRENT` + objeto `subscription`), cartão salvo lá, cobrança automática, `callback` e webhooks. **`externalReference` (200 chars) é o equivalente do `metadata.userId` da Stripe** — resolve o "o webhook não pode descobrir o dono pelo e-mail" do §8.
  - 🔴 **Achado que decide a T-216:** **não existe caminho PCI-limpo via API para trocar o cartão.** `PUT /v3/subscriptions/{id}/creditCard` aceita token **ou** dado bruto, mas ambos exigem chamada de servidor com API key, e o token só nasce do endpoint que recebe o PAN. **Não há campos hospedados/iframe** (equivalente do Stripe Elements). → **O portal (T-216) é nosso mas NUNCA toca em cartão:** mostra status/faturas/histórico e **redireciona para um checkout hospedado novo** para qualquer ação com cartão. Pior UX que o Customer Portal, e é o preço consciente de não assumir PCI.
  - 🔴 **Entrada obrigatória para a T-208:** **o checkout hospedado NÃO aceita boleto** (`billingTypes` = `CREDIT_CARD` e `PIX`). Boleto existe, mas por outro fluxo (`POST /v3/subscriptions` com `billingType: BOLETO`, sem cartão, sem PCI). **Liberar boleto = manter DOIS caminhos de cobrança**, não marcar uma caixinha. E 🔬 **Pix recorrente é dúvida aberta** — o exemplo de checkout recorrente só mostra cartão.
  - 🔴 **Regressão de segurança para a T-214:** o webhook do Asaas autentica por **`asaas-access-token` estático no header** — **não** é assinatura sobre o corpo cru, como Stripe e Resend. É mais fraco que os dois que já temos. Entrega "at least once" (idempotência pelo `id` do evento, como já fazemos) e **a fila pode ser interrompida após falhas seguidas** → alerta de silêncio (T-223) deixa de ser zelo. ⚠️ O `rawBody: true` do `main.ts` existe **para a Stripe**: ao desligá-la (T-224), não o remova sem verificar quem mais depende dele.
...
- [x] **T-208 — Definir os meios de pagamento** 🔴 (bloqueadora) — **DECIDIDA em 30/07/2026 (dono): CARTÃO + BOLETO + PIX.** Revoga a decisão de "só cartão" do `CLAUDE.md` §9, que foi tomada no contexto da Stripe. **§9 atualizado no mesmo commit.**
  - **Como cada meio entra (não são o mesmo caminho — este é o ponto que a T-207 descobriu):**
    ...
    - **Pix — ✅ RESOLVIDO na T-209 (30/07), contra o sandbox.** O checkout hospedado **recusa** Pix em `RECURRENT` (`O tipo de cobrança DETACHED é obrigatório para o método de pagamento PIX`), **mas `POST /subscriptions` aceita `billingType: PIX`** e cria assinatura `ACTIVE`. **Pix recorrente existe — pelo mesmo caminho do boleto.** ⚠️ Exige **chave Pix criada na conta** (pendência de painel do dono).
  ...
  - **⚠️ Impacto dimensionado (o que a task pedia), e ele NÃO é pequeno:**
    ...
    - ✅ **A T-209 ENCOLHEU este impacto: são DOIS caminhos, não três.** `POST /subscriptions` aceita `billingType: UNDEFINED`, em que **o pagador escolhe o meio a cada cobrança** — ou seja, boleto **e** Pix cabem num caminho só. O desenho vira: **checkout hospedado (cartão)** + **assinatura direta `UNDEFINED` (boleto/Pix)**. A T-213 e a T-216 continuam crescendo em relação ao "só cartão", mas menos do que este parágrafo previa.
    - **⚠️ A estimativa global do épico (8-12 dias) segue pendente de revisão** — foi feita para o escopo original.
...
- [x] **T-209 — Conta Asaas, sandbox e credenciais** 🔴 (bloqueadora) — **FEITA em 30/07/2026. Sandbox respondendo a partir da aplicação local; medições completas em `spikes/RESULTADOS.md` (seção T-209).**
  - **Entregue:** sandbox autenticado (`GET /customers` → 200); envs `ASAAS_BASE_URL`, `ASAAS_API_KEY` e `ASAAS_WEBHOOK_TOKEN` declaradas no `render.yaml` com `sync: false` e documentadas no `.env.example`; chave **só** em `apps/api/.env` (gitignored, conferido) — **nenhuma credencial versionada**. Dois spikes reproduzíveis (`asaas-sandbox.mjs`, `asaas-sandbox2.mjs`) que **nunca imprimem a chave**.
  - 🔴 **DERRUBOU A PREMISSA CENTRAL DO ÉPICO — "o Asaas exige CPF/CNPJ para criar cliente" é FALSO na letra.** `POST /customers` sem `cpfCnpj` responde **200** (`cpfCnpj: null`). O documento é exigido **na COBRANÇA**: `POST /payments` e `POST /subscriptions` recusam com 400 `Para criar esta cobrança é necessário preencher o CPF ou CNPJ do cliente.`
    - **A substância se mantém:** conta sem CNPJ segue **incobrável** — só falha em outro momento. **Isso VALIDA a decisão da T-225** (coletar no cadastro/onboarding + exigir ao assinar): a falha cairia exatamente no ato de assinar.
    - ⚠️ **Consequência para a T-212:** dá para criar/sincronizar o cliente Asaas **cedo, sem documento**. Não é preciso ter CNPJ para começar a sincronizar — só para cobrar.
  - 🔴 **DERRUBOU O DIMENSIONAMENTO DA T-208 — são DOIS caminhos, não três, e o Pix cabe junto do boleto.** O checkout hospedado é categórico (mensagem literal da API): `O método de pagamento CREDIT_CARD é o único método de pagamento permitido para operações RECURRENT` e `O tipo de cobrança DETACHED é obrigatório para o método de pagamento PIX`. **Mas `POST /subscriptions` aceita `billingType: PIX` (ACTIVE ✓) e `UNDEFINED` (ACTIVE ✓).**
    - ✅ **Responde a dúvida 🔬 da T-208: Pix recorrente EXISTE** — só não pelo checkout.
    - 📌 **Desenho recomendado para a T-213:** (a) **checkout hospedado** para cartão (renovação automática, PCI SAQ A) + (b) **assinatura direta com `UNDEFINED`**, cobrindo **boleto e Pix num caminho só**, com o cliente escolhendo o meio a cada cobrança. Menos do que a T-208 dimensionou.
  - ✅ **`endDate` NÃO é obrigatório** no checkout `RECURRENT` (testado sem, aceito). O exemplo da doc traz `endDate` e induzia a gravar um fim artificial numa assinatura de SaaS, que não acaba.
  - 🔴 **Webhook sem HMAC — CONFIRMADO por medição** (a T-207 tinha lido na doc). Campos do objeto: `id, name, url, email, enabled, interrupted, apiVersion, hasAuthToken, sendType, penalizedRequestsCount, events, authToken`. **Nenhum campo de assinatura.** O `authToken` exige **mínimo 32 caracteres** e **volta legível pela API** (quem tem a API key lê o token). ⚠️ **`interrupted` e `penalizedRequestsCount` são campos de primeira classe: a fila PARA após falhas seguidas** — isso promove o alerta de silêncio (T-223) de zelo a **necessidade**.
  - ⚠️ **DUAS PENDÊNCIAS DE PAINEL (dono, não código):**
    1. **Criar uma chave Pix na conta Asaas** — sem ela o Pix não gera cobrança (`Para gerar cobranças com Pix é necessário criar uma chave Pix no Asaas`). O meio já foi escolhido na T-208; falta habilitar.
    2. **NFS-e não configurada:** `GET /invoices/municipalSettings` → **404**. Esperado — depende do CNPJ e do município, que é o bloqueio externo conhecido da T-219.
  - **Ambiente, para quem repetir:** sandbox é **conta separada** (`sandbox.asaas.com`, login próprio); chave de sandbox tem prefixo **`$aact_hmlg_`**, produção `$aact_prod_`; header é **`access_token`** (sem `Bearer`); **`User-Agent` obrigatório**. ⚠️ Trocar chave e host dá **401**, que **não** diz "ambiente errado".
  - **Escopo original, para referência:**
...
- [x] **T-211 — Modelo de dados e migration** 🟠 — **FEITA em 30/07/2026.** Migration `1786900000000-AddAsaasBilling`.
  - **Entregue:** em `assinaturas`, as colunas `asaas_customer_id`, `asaas_subscription_id`, `provider` e `asaas_atualizado_em`; índice **ÚNICO** em `asaas_subscription_id` e comum em `asaas_customer_id`; tabela **`asaas_events`** (PK = id do evento) espelhando `stripe_events`. **Nenhuma coluna da Stripe foi apagada**, como a task mandava.
  - ✅ **`up`, `down` e `up` de novo executados contra o Postgres real** (não "não rodada ao vivo", como em tasks anteriores). Três propriedades provadas por SQL, em transação com `ROLLBACK`:
    1. **Coexistência:** uma linha com `stripe_subscription_id` de histórico **e** `asaas_subscription_id` ativo, com `provider='asaas'` dizendo quem cobra — que é literalmente o "Pronto quando" desta task.
    2. **Vários NULL convivem** no índice único (é o estado de quem está em trial).
    3. **Duplicata é recusada:** `duplicate key value violates unique constraint "idx_assinaturas_asaas_sub"`.
  - ✅ **Índice GIN conferido depois do ciclo `down`/`up`** — segue de pé (dívida §10.1: o `migration:generate` costuma derrubá-lo; esta migration é escrita à mão e não o toca).
  - 📌 **`provider` é NULLABLE e nasce NULL para quem está em trial** — o trial é NOSSO (T-127) e não existe em provedor nenhum. O `up` faz backfill: quem já tem `stripe_subscription_id` vira `'stripe'`. ⚠️ **Não deduza o provider pela presença dos ids:** depois do corte (T-224) uma conta pode ter o id da Stripe como **histórico** e estar sendo cobrada pelo Asaas.
  - 🔴 **`asaas_payment_id` ficou de FORA, contra o escopo original — decisão consciente.** Uma assinatura tem N cobranças, então guardar "a" cobrança na assinatura é **espelho de estado do provedor**, justamente o que o `stripe-billing.service.ts` evita por escrito ("um espelho dessincronizado mostraria ao cliente um dado que não é mais o dele"). E o webhook **não precisa dele** para localizar a assinatura — o evento traz o id da assinatura. **Se a T-216 provar que a tela precisa da cobrança em aberto** (linha digitável do boleto, QR do Pix), ela vem por leitura ao vivo ou por coluna própria, aí com motivo medido.
  - ⚠️ **`criado_em_asaas` é NULLABLE**, ao contrário do equivalente da Stripe: o payload do Asaas não garante data em todo tipo de evento. **Aceitar sem carimbo é melhor que recusar** — recusar seria perder uma cobrança confirmada.
  - ⚠️ **O "restore de backup validado depois da migration" NÃO foi feito** — a rotina de backup **não existe** (o épico a lista como pré-requisito sem dono). Com **zero pagantes** o risco hoje é baixo, mas isto **continua em aberto** e fica mais caro a cada task de billing.
  - 📌 **Achado de passagem, não corrigido (§4.3):** o webhook da Stripe busca a assinatura por **`stripe_customer_id`** na maioria dos caminhos, e **não há índice nessa coluna** (só em `stripe_subscription_id`). É seq scan. Não corrigi porque a tabela é pequena e a coluna **sai na T-224** — indexar o que está de saída é desperdício. Nos campos do Asaas os **dois** estão indexados.
...
- [x] **T-212 — Cliente Asaas e coleta obrigatória de CNPJ** 🔴 — **FEITA em 30/07/2026** (a metade "coleta" saiu antes, na T-225).
  - **Entregue:** `asaas-client.ts` (HTTP fino, **sem SDK** — `fetch` nativo, como o Resend; zero dependência nova), `asaas.provider.ts` (`ASAAS_CLIENT`, `null` sem env → **503**, mesma degradação da Stripe/IA/Google) e `asaas-billing.service.ts` com `garantirCustomer`. Registrados no módulo; **nenhum controller os chama ainda** — quem cobra em produção segue sendo a Stripe até a T-224.
  - ✅ **Verificado contra o SANDBOX REAL, não só com mock:** `GET` e `POST` funcionando, recuperação por `externalReference` batendo o id, e a tradução de erro devolvendo `AsaasError` com status 400, código `invalid_object` e a mensagem "O CPF/CNPJ informado é inválido". **10 testes** novos (suíte 970 → **980**), lint e build limpos.
  - 🔴 **`externalReference` não é enfeite — é o conserto da falta de idempotência.** O Asaas **não tem chave de idempotência** como a Stripe. Se uma tentativa cria o cliente e morre antes de gravar o id, a retentativa criaria um **segundo cliente para a mesma pessoa**. Por isso o serviço **busca por `externalReference` antes de criar** e adota o órfão (com log). Medido no sandbox: o filtro `GET /customers?externalReference=` funciona e devolve o id certo.
  - 📌 **`provider` NÃO é escrito ao criar o cliente** — cliente não é cobrança. Marcar aqui faria o `/admin` e a reconciliação acharem que a conta migrou quando ela só tem cadastro do outro lado. Quem decide isso é a **T-213**, quando a assinatura existir. Há teste guardando essa ausência.
  - 📌 **A barreira do CNPJ é NOSSA, e é deliberadamente mais cedo que a do provedor.** A T-209 mediu: o Asaas cria cliente sem documento (200) e só recusa em `/payments` e `/subscriptions`, com uma mensagem crua que não ajuda o usuário. `garantirCustomer(..., { exigirDocumento: true })` barra antes, com texto que diz o que fazer. **Sem documento e sem cobrar, o cliente é criado normalmente** — não há motivo para bloquear cadastro.
  - ⚠️ **Documento divergente NÃO é sobrescrito.** Se o Asaas tem um CNPJ e nós temos outro, o serviço **loga como erro e não escreve** — trocar o documento de um cliente é mudar a identidade fiscal de quem pode já ter nota emitida (T-219). Mesma regra do `setCnpj` (T-225): preenche vazio, não troca preenchido.
  - ⚠️ **A chave e a base URL formam PAR, e o provider avisa em log quando não batem** (`$aact_hmlg_` no host de produção, ou o inverso). O sintoma real desse engano é **401**, que não diz "ambiente trocado" — o aviso existe porque isso custa uma tarde de quem não foi avisado.
  - ✅ **A COLETA já está feita (T-225, 30/07/2026).** Sobrou aqui só o que depende de fato do Asaas: criar o cliente na hora certa do funil, mantê-lo sincronizado, e **exigir CNPJ de quem ainda não tem no momento de assinar** (as contas legadas). A coleta em si — cadastro, onboarding e endpoint — não é mais escopo desta task.
  - 🔴 **CORRIGIDO pela T-209 (30/07, medido):** o Asaas **NÃO** exige documento para criar cliente (`POST /customers` sem `cpfCnpj` → 200, `cpfCnpj: null`). Ele exige **na COBRANÇA** — `POST /payments` e `POST /subscriptions` recusam com 400. **A conclusão prática não muda** (conta sem CNPJ é incobrável, e a Stripe não exigia nada disso), mas **o momento da quebra sim**: dá para criar e sincronizar o cliente cedo, sem documento, e só barrar ao cobrar.
...
#### Fora das fases — entregue durante a Fase 0

- [x] **T-225 — CNPJ obrigatório no funil (pré-requisito da T-212)** 🔴 — **FEITA em 30/07/2026.** *(Numeração: o épico ia até T-224; esta nasceu depois, por isso T-225.)*
  - **Por que virou task própria em vez de ficar na T-212:** não depende de **nada** do Asaas (nem da T-210, nem da T-211), e vale sozinha — é a dívida §10.10, furo de qualidade de dado num B2B onde o CNPJ é a identidade do cliente. Fazê-la na Fase 0 tirou trabalho do caminho crítico enquanto a conta Asaas (T-209) não existe.
  - **Decisões do dono (30/07), as duas registradas porque mudam o produto:**
    1. **Onde exigir:** contas **novas** (cadastro por e-mail + onboarding) **e no ato de assinar** para quem não tem. **Sem gate bloqueante** para o legado — quem está em trial não é incomodado; o Asaas só precisa do CNPJ na hora de cobrar.
    2. **"Um CNPJ = uma conta" MANTIDO.** O índice único já existia em `users.cnpj`, mas era inofensivo porque quase todo valor era `null` (Postgres permite vários nulos num índice único). Com o campo obrigatório isso virou **regra dura de negócio**: o 2º sócio da mesma construtora **não** cria conta própria. Consequência aceita conscientemente — e coerente com a T-87 (Equipe & convites), que foi descartada.
  - **Entregue:**
    - **Cadastro por e-mail:** `cnpj` deixou de ser `@IsOptional()` no `RegisterDto` e ganhou `@Transform` (só dígitos) — o servidor não pode depender do formato que o front mandou. Front: campo `required`, com mensagens distintas para "não informou" e "informou errado".
    - **Cadastro pelo Google:** o onboarding passou a coletar o CNPJ, **só para quem chegou sem ele** — mesmo padrão que a UF já usava (`ufCadastrada`). Novo `PUT /users/me/cnpj`, espelhando o `PUT /users/me/uf`. O campo entrou no rascunho da T-167, senão o F5 o perderia.
    - **`setCnpj` só PREENCHE quando está vazio; não troca.** Trocar o CNPJ é mudar a **identidade fiscal** da conta — é ele que vai para a cobrança e para a NFS-e (T-219). Deixar isso num PUT do próprio usuário permitiria alterar o documento da nota **depois de emitida**. Alteração passa por suporte/admin. Reenviar o mesmo valor é no-op (idempotente).
  - 🔴 **Bug pré-existente que a mudança teria promovido a rotina:** o `register` tratava e-mail duplicado com 409 mas **não tratava CNPJ duplicado** — a violação do índice subia como **500**. Com o campo obrigatório esse caminho deixaria de ser raro. Agora vira **409 com mensagem útil** ("Este CNPJ já está cadastrado. Se a empresa já tem conta, entre com ela."), via `common/pg-erros.ts`.
    - ⚠️ **Conferir antes de gravar seria uma CORRIDA** (duas requisições simultâneas passariam as duas pela checagem). **Quem decide é o índice; o código só traduz o erro.** O helper olha o código `23505` do driver, **não a mensagem** — mensagem muda com versão e idioma do Postgres.
    - ⚠️ Há teste garantindo que erro que **não** é violação de unicidade **continua subindo** — senão a tradução esconderia falha de infraestrutura atrás de mensagem de negócio.
  - **Verificação:** API **970 testes** verdes (era 922; +48 entre regras novas e fixtures), front **131** verdes, lint e build limpos nos dois pacotes. ⚠️ **Falta (§4.4): sign-off no navegador** — cadastrar com CNPJ repetido e ver o 409, e passar pelo onboarding de uma conta Google sem CNPJ.
  - 📌 **Não incluído de propósito:** a exigência **no ato de assinar** ficou para a **T-212**, onde o cliente Asaas é criado. Construí-la agora contra a Stripe, que está sendo substituída, seria trabalho jogado fora. E **a base legada não foi auditada** (quantas contas estão sem CNPJ) — isso precisa do banco de produção.

- [x] **T-213 — Assinatura e conversão do trial** 🟠 — **FEITA em 30/07/2026.**
  - **Entregue:** `criarCheckout` (cartão, `POST /checkouts` com `CREDIT_CARD`+`RECURRENT`) e `criarAssinaturaDireta` (boleto/Pix, `POST /subscriptions` com `billingType: UNDEFINED`) no `AsaasBillingService`; **preço no config store** com endpoint `PUT /admin/config/precos` (auditado) e **card na tela de Config do `/admin`**. API **989 testes** verdes (era 980), front 131, lint e build limpos nos dois.
  - 🔴 **O risco desta task era de UNIDADE, e está isolado num lugar só.** O projeto inteiro fala **centavos** (herança da Stripe); **o Asaas fala REAIS** — `value: 100` é R$ 100,00. Mandar centavos direto **cobra 100× do cartão do cliente**. A conversão é a função exportada `centavosParaReais`, com teste próprio (inclusive contra dízima de ponto flutuante). **Se for preciso converter em outro lugar, use essa função — não escreva `/100` de novo.**
  - 🔴 **O PREÇO agora mora no config store (`precos_assinatura`) — revoga a regra do §8.** Não é preferência: o Asaas **não tem catálogo de `Price`**, a assinatura carrega um `value` que nós mandamos, então não existe "lá" de onde ler. O **espírito** da regra sobrevive — muda **sem deploy** e com **registro de quem mudou** (auditoria + `updatedByAdminId`).
    - ⚠️ **Sem preço configurado → 503, nunca um valor inventado.** Falhar fechado é a única opção aceitável no dinheiro. **O dono precisa definir o preço em `/admin` → Config antes de qualquer cobrança pelo Asaas funcionar** — e a tela avisa isso em laranja quando está vazio.
    - ⚠️ **Duas camadas de faixa** (DTO e store), de propósito: preço é caminho do dinheiro e uma camada só é pouco. Fora da faixa o store **recusa** em vez de "corrigir" — diferente do trial, ajustar preço em silêncio faria o sistema cobrar um número que ninguém digitou.
  - ✅ **`nextDueDate` e a AUSÊNCIA de `endDate` verificados contra o sandbox**, além do formato de resposta do checkout: o campo do link chama-se **`link`** (confirmado com chamada real, não presumido do doc).
  - 📌 **`provider = 'asaas'` é escrito AQUI**, não na criação do cliente (T-212): é neste ponto que a cobrança passa a existir do outro lado.
  - 🔴 **NADA nesta task altera `status` da assinatura** — há teste garantindo. Quem libera acesso é o webhook (T-214), pela mesma razão da Stripe (§8): retorno de navegador não prova pagamento, o usuário pode digitar a `successUrl` na barra. ⚠️ O `status: ACTIVE` que o Asaas devolve ao criar a assinatura significa **"a assinatura existe"**, não "foi paga" — não o confunda com liberação de acesso.
  - ✅ **O trial continua sem tocar no provedor** (o "não regredir" do Pronto quando): nada é criado no Asaas enquanto a pessoa só experimenta. Conversão é o primeiro momento em que o Asaas fica sabendo que a conta existe.
  - **Decisão do dono (30/07): boleto/Pix valem nos DOIS planos**, não só no anual. A conta refeita com números: R$ 3,49 fixo num mensal de ~R$100 dá ~3,5%, praticamente igual ao cartão (2,99% + R$ 0,49) — **o boleto mensal não é tão pior quanto a T-208 sugeria**. O custo real é operacional (12 cobranças manuais/ano por cliente), e ele cai na **T-220**.
  - ⚠️ **Falta (§4.4):** sign-off no navegador do card de preço em `/admin` → Config.
...
- [x] **T-214 — Webhooks Asaas** 🔴 — **FEITA em 30/07/2026.** `POST /webhooks/asaas` público + `AsaasWebhookService`. API **1005 testes** verdes (era 989; +16), lint e build limpos.
  - **É AQUI, e só aqui, que alguém vira `active`** — mesma regra da Stripe (§8): retorno de navegador não prova pagamento.
  - 🔴 **BUG DE FUSO ACHADO E CORRIGIDO — o teste pegou, não a leitura.** O Asaas manda `dateCreated` como `"2026-07-30 10:00:00"`, **sem fuso, em horário de Brasília**. `new Date(texto.replace(' ','T'))` interpreta como hora **local do servidor**, e o nosso roda em **UTC** (Render, Oregon): **3 horas de erro em cada carimbo**.
    - **Por que não é inofensivo, mesmo com todos os eventos igualmente deslocados:** o `asaasAtualizadoEm` também recebe `now` (relógio real, UTC) quando o evento chega sem data. Aí um evento legítimo parece 3h mais velho que o estado e é **descartado pela guarda de ordem — quem pagou não é liberado**. Misturar dois relógios no mesmo campo é a receita do bug silencioso.
    - **Correção:** a função pura `dataAsaas` fixa `-03:00`. Seguro porque o Brasil **aboliu o horário de verão em 2019**; se voltar, é o ponto único a mudar. Três testes guardam isso. **É o mesmo tipo de erro que o projeto já teve na exibição de datas (§6) — a lição é que timestamp sem fuso é bomba-relógio.**
  - 🔴 **Autenticação é uma REGRESSÃO em relação à Stripe, e o código diz isso:** não há assinatura sobre o corpo — só o `asaas-access-token` **estático** (T-209). Consequências tratadas: comparação em **tempo constante** (`timingSafeEqual`, porque `===` vaza o prefixo correto pelo tempo e o token é a única barreira); **não** depende de `rawBody` (o `rawBody: true` do `main.ts` é da Stripe); e token errado → **401**, não reentrega.
  - ⚠️ **Env ausente → webhook DESABILITADO respondendo 200**, como o do Resend. Devolver erro faria o Asaas reentregar até **INTERROMPER a fila** — e a fila dele para mesmo (`interrupted`, `penalizedRequestsCount`).
  - 📌 **Decisões de mapeamento que o nome do evento não revela:**
    - `PAYMENT_CONFIRMED` **e** `PAYMENT_RECEIVED` liberam acesso. `CONFIRMED` é "pagou", `RECEIVED` é "o dinheiro caiu na conta" — esperar o repasse puniria o cliente por tesouraria do provedor.
    - `PAYMENT_PARTIALLY_REFUNDED` **NÃO corta acesso**: a regra do projeto (T-157) é que só o reembolso **integral** desfaz a assinatura.
    - `PAYMENT_OVERDUE` **preserva** o `pastDueDesde` original — a carência (T-130) conta do PRIMEIRO vencimento; sobrescrever a cada evento a reiniciaria e o inadimplente teria acesso para sempre.
    - Evento **sem assinatura nossa** é ignorado com log, não com erro: o Asaas manda eventos de toda a conta (o dono pode cobrar alguém pelo painel), e lançar faria a fila ser interrompida.
  - ⚠️ **O fim do período vem de uma chamada extra** (`GET /subscriptions/{id}` → `nextDueDate`): o evento de pagamento **não traz** essa data, e calcular "dueDate + 1 ciclo" duplicaria a regra de calendário do provedor (mês de 28/31 dias, pagamento adiantado). **Se essa leitura falhar, o pagamento ainda é registrado** — melhor período desatualizado que perder a confirmação.
  - ⚠️ **Falta para funcionar de fato:** cadastrar o endpoint no painel do Asaas apontando para `https://api.prumolicita.com.br/webhooks/asaas` e setar `ASAAS_WEBHOOK_TOKEN` (**mínimo 32 caracteres**) nos dois lados. **Sem sign-off de ponta a ponta com evento real** (§4.4).
  - 🔴 **Medido na T-209 (30/07), antes de escrever uma linha:** a autenticação é **só** o `authToken` estático no header `asaas-access-token` — **não existe campo de HMAC/assinatura** no objeto webhook (campos: `id, name, url, email, enabled, interrupted, apiVersion, hasAuthToken, sendType, penalizedRequestsCount, events, authToken`). É **mais fraco** que a Stripe (assinatura sobre corpo cru) e que o Resend (Svix). Token exige **≥32 caracteres** e **volta legível pela API**.
  - ⚠️ **A fila PARA sozinha:** `interrupted` e `penalizedRequestsCount` são campos de primeira classe — falhas seguidas interrompem a entrega. **Sem alerta de silêncio (T-223) a cobrança para de chegar e ninguém fica sabendo.** Tratar T-223 como parte do caminho crítico, não como polimento.
  - ⚠️ **O `rawBody: true` do `main.ts` existe para a STRIPE.** Sem assinatura sobre corpo cru, o Asaas não precisa dele — mas **não o remova ao desligar a Stripe (T-224)** sem conferir quem mais depende (§8).
...
- [x] **T-215 — Paywall (402) sobre o estado unificado** 🟠 — **FEITA em 30/07/2026, com ZERO linhas de código de produção alteradas.** A previsão do épico ("esta task é quase só teste de matriz") **se confirmou** — e o valor entregue foi transformar isso de suposição em evidência.
  - **O que foi VERIFICADO, não construído:** `acesso.ts`, `subscription.guard.ts` e `assinaturas.service.ts` não contêm **nenhuma** referência a `stripe`, `asaas` ou `provider` — a decisão de acesso sempre leu só a NOSSA entidade. O 402 (`motivo` + `redirect: '/assinatura'`) segue intacto, então **não há mudança de contrato para o front**, que era o requisito.
  - **O que foi ACRESCENTADO: 5 testes que provam a agnosticidade** (suíte `acesso.spec` 22 → 27; total da API **1010**). "É agnóstico" sem teste é a classe de afirmação que deixa de valer no primeiro campo novo que alguém lê ali dentro — agora, se a decisão passar a olhar `stripeSubscriptionId` ou `provider`, os testes quebram.
    - Mesmo estado de negócio × dois provedores → decisão **idêntica** (`toEqual`), com os campos específicos de cada um preenchidos de propósito.
    - **Conta migrada** (histórico Stripe **e** assinatura Asaas ao mesmo tempo, o cenário que a T-211 tornou possível): decide pelo estado. Se olhasse "tem id da Stripe?", erraria exatamente aqui.
    - **Trial com `provider: null`** — o estado de quem nunca pagou, porque o trial é nosso e não existe em provedor nenhum (T-127/T-213).
  - **A matriz do "Pronto quando" já estava coberta** pelos 22 testes existentes (trial ativo/expirado, ativa, past_due dentro/fora da carência, cancelada com/sem período em aberto, sem assinatura, cortesia, suspensão, reembolso) — **não os dupliquei**. O guard também já tinha os três casos (passa, 402 com motivo e rota, e não-autenticado que é 401 e não 402).
  - 📌 **Por que isso é bom sinal e não trabalho pulado:** o paywall foi construído em julho lendo estado próprio em vez de perguntar à Stripe (§3.3 — "cálculo no backend, front só renderiza"). **Trocar o provedor inteiro não tocou nele.** É a decisão de arquitetura de julho pagando dividendo agora.
...
- [x] **T-216 — Portal do assinante** 🔴 — **FEITA em 31/07/2026 — backend, tela e SIGN-OFF NO NAVEGADOR (§4.4) pelo dono.** *(o item mais caro do épico)*
  - 🔴 **Achado que ENCOLHE muito a task:** o Asaas devolve **`invoiceUrl`** em cada cobrança — uma **página de pagamento hospedada** que serve boleto **e** Pix, inclusive quando o `billingType` é `UNDEFINED` e o pagador escolhe na hora. **Não precisamos renderizar linha digitável nem QR de Pix.** Medido no sandbox (as três assinaturas de teste devolveram `invoiceUrl`; as de boleto também `bankSlipUrl` com o PDF). Isso tira da tela a parte cara e mantém a coerência de PCI: **nenhum instrumento de pagamento passa por nós**.
  - ✅ **Entregue (backend):** `detalhesPortal(userId)` no `AsaasBillingService` — lê a assinatura local, lista as últimas 12 cobranças e mapeia para a forma que a tela precisa (valor **em centavos**, vencimento, status cru, meio, `pagarUrl`, `boletoUrl`, `comprovanteUrl`). **7 testes** novos; suíte da API **1010 → 1017**.
    - **`reaisParaCentavos`, a volta da fronteira de unidade.** A T-213 cuidou da ida; a leitura cruza de volta, e sem ela o valor apareceria **100× menor** na tela — erro silencioso, porque ninguém reclama de ver preço baixo demais até a fatura chegar. Há teste de **ida e volta**.
    - **`temGestaoExterna: false`** existe para o front escolher entre "abrir portal do provedor" (Stripe) e "renderizar nossa tela" (Asaas) **sem adivinhar**.
    - **Provedor instável não derruba a tela:** falha ao listar cobranças devolve lista vazia e loga, em vez de estourar — senão uma indisponibilidade do Asaas deixaria o assinante sem ver o próprio plano.
    - ⚠️ **`comprovanteUrl` NÃO é NFS-e.** A nota é a T-219. Rotulá-lo de "nota fiscal" na tela prometeria documento fiscal que o cliente não recebe ali — mesmo cuidado que o `reciboUrl` da Stripe já tem (§8).
  - ✅ **DECISÃO DO DONO (30/07): a troca de plano acontece NA VIRADA DO CICLO, sem proporcional** — e `trocarPlano` já está implementado (5 testes).
    - **Três razões, em ordem de peso:** (1) mensal e anual **não diferem em funcionalidade**, só em preço e ciclo — não existe "upgrade" a apressar, e o desconto do anual começa quando a cobrança anual começa; (2) **o Asaas não faz rateio** (a Stripe fazia), então proporcional seria matemática de dinheiro em código nosso, sem backup com restore testado; (3) **o downgrade não teria solução limpa** — quem pagou o ano teria crédito a receber, e o Asaas não tem conceito de saldo.
    - 🔴 **`updatePendingPayments: false` é o coração disto, e foi MEDIDO no sandbox:** troquei uma assinatura de R$100/mês para R$1499/ano e a cobrança **já gerada** continuou em R$100, com `nextDueDate` intacto. ⚠️ **Passar `true` reescreveria uma cobrança que o cliente pode já estar pagando — inclusive um boleto já impresso, com outro valor.** Há teste travando o `false`, porque é a linha que alguém "melhora" sem entender.
    - ⚠️ **A tela PRECISA mostrar a data (`valeAPartirDe`) junto do nome do plano:** sem ela, "plano anual" mente sobre a cobrança em aberto, que segue no valor antigo.
  - ✅ **TELA FEITA (31/07):** `PortalAssinanteCard.tsx` com `CobrancasCard` (cobrança em aberto destacada no topo com botão **Pagar** → `invoiceUrl`, link do **boleto em PDF**, e histórico em tabela com status traduzido) e `TrocarPlanoCard` (troca com a **data em que passa a valer**). Endpoints `GET /assinaturas/portal` e `POST /assinaturas/plano`.
    - 🔴 **A rota do portal é PROVIDER-AWARE, e isso evita um flag day:** ela devolve `temGestaoExterna: true` para Stripe/trial (a tela segue usando o botão do Customer Portal, como hoje) e `false` para Asaas (renderiza a tela nova). **Nada muda para nenhum usuário atual** — hoje ninguém tem `provider = 'asaas'`. Na T-224, a virada é de dado, não de código.
    - ⚠️ **A tela NUNCA toca em cartão** (T-207): pagar leva à página hospedada do provedor, que serve boleto **e** Pix. Formulário de cartão aqui subiria o escopo de **SAQ A para SAQ A-EP**.
  ...
  - ✅ **TROCA DE CARTÃO IMPLEMENTADA (31/07) — e ela ACEITA O ESCOPO PCI SAQ A-EP (decisão do dono).**
    - **Por que a decisão mudou:** o Asaas **tem** troca de cartão (`PUT /subscriptions/{id}/creditCard`, sem cobrar na hora) — o que ele não tem é como fazê-la sem o cartão passar por um servidor nosso: a tokenização é **server-side** (autenticada pela API key) e **não há SDK JS com chave pública** como o Stripe.js. Ou seja, não era limitação do provedor, era escolha de conformidade. Com o custo visível (cartão vencido sem conserto self-service) o dono optou por assumir o escopo.
    - ⚠️ **O QUE ISSO OBRIGA, e não se desfaz:** SAQ A (≈22 perguntas, nenhuma obrigação técnica) vira **SAQ A-EP (≈139)** — desenvolvimento seguro documentado, política de logs e retenção, segregação de ambiente, resposta a incidente e **varredura trimestral por scanner aprovado (ASV)**. **A aplicação INTEIRA entra no escopo**, não só a tela do cartão.
    - ⚠️ **A tokenização precisa ser habilitada pelo gerente da conta Asaas para PRODUÇÃO** (no sandbox já vem ligada).
    - **INVARIANTES do caminho do cartão, cada uma com teste:** nada de cartão é **persistido**; nada vai para **log** (verificado: `grep` do número e do CVV no log da API depois de uma troca real → **zero ocorrências**); nada volta na **resposta** (só últimos 4 + bandeira); e o `remoteIp` é o do **cliente**, exigência antifraude, vindo da função única da T-204.
    - ✅ **Validado ponta a ponta contra o sandbox:** `PUT /assinaturas/cartao` → **200** devolvendo `{ultimos4, bandeira}`; corpo inválido → **400**, barrado pelo DTO **antes** de sair para o provedor.
    - 📌 **Consequência que vale avaliar depois:** com SAQ A-EP aceito, dá para criar a assinatura **direto com cartão** (`POST /subscriptions` com `creditCard`), dispensando o checkout hospedado — o que resolveria de uma vez os **três** problemas que ele causou (cliente fantasma, assinatura duplicada e documento errado na nota). **Não foi feito nesta task.**
  - 🔴 **Histórico: o botão anterior de trocar cartão foi REMOVIDO em 31/07 — bug com dinheiro. A causa continua valendo como aviso.**
    - **O que acontecia:** o botão abria um **checkout hospedado novo**. Em modo recorrente, o checkout do Asaas **CRIA UMA ASSINATURA** — não atualiza o cartão da existente. Resultado medido no sandbox: **duas assinaturas ATIVAS**, em dois clientes diferentes, **as duas cobrando**, mais uma cobrança imediata. E a antiga ficava **órfã e invisível** para o sistema, porque o nosso banco passava a apontar para a nova (a troca de plano seguinte foi parar na assinatura errada — foi assim que o dono percebeu).
    - **Por que não há substituto:** o único endpoint que troca o cartão de uma assinatura existente é `PUT /subscriptions/{id}/creditCard`, e ele exige o cartão — ou um `creditCardToken` que só nasce mandando o PAN a um servidor nosso. Isso sobe o escopo de **SAQ A para SAQ A-EP** (T-207 já tinha medido; **o erro foi meu, ao supor que um checkout hospedado novo servia de substituto**).
    - 🔴 **CONSEQUÊNCIA DE PRODUTO QUE PRECISA DE DECISÃO DO DONO: cartão vencido não tem conserto self-service hoje.** O cliente cai em `past_due` e o caminho é cancelar e assinar de novo, ou suporte. É argumento real a favor de **boleto/Pix como meio principal** — ou de reabrir a conversa sobre tokenização, aceitando SAQ A-EP.
    - 📌 **Lição que vale além desta task:** "não existe caminho PCI-limpo por API" (T-207) **não** implica "então use um checkout novo". Checkout recorrente é *criação*, não *edição* — e a diferença só apareceu quando alguém pagou de verdade.
  ...
  - 🔴 **CLIENTE FANTASMA no Asaas — corrigido em 31/07, achado pelo dono na lista de clientes.** Apareciam **dois clientes com o mesmo e-mail**: um criado por nós (`garantirCustomer`) e outro criado pelo checkout. **Não era o nome** — é estrutural: o `POST /v3/checkouts` **não aceita vincular cliente existente**, então ele sempre cria o dele com o que o pagador digita, e o nosso ficava sem uso nenhum. **Correção:** o caminho do cartão **não cria mais cliente** — só valida o CNPJ (a cobrança falharia sem ele). O caminho boleto/Pix continua criando, porque lá o cliente é usado de fato.
  - 🔴 **RISCO ABERTO PARA A T-219 (NFS-e), e precisa de decisão do dono:** sem pré-preencher, **o cliente criado pelo checkout de cartão fica com o documento que o PAGADOR digitar** — na prática, o CPF dele. **A nota sairia no CPF, e a construtora precisa dela no CNPJ** para lançar como despesa, que é o motivo deste épico existir.
    - **Por que não dá para simplesmente pré-preencher:** o `customerData` é **tudo ou nada** — testado contra o sandbox, recusa com 400 exigindo `phoneNumber`, `address`, `addressNumber`, `postalCode` e `province`. **Não guardamos endereço** (o `company_profile` tem razão social e telefone).
    - **Saídas:** (a) coletar endereço no perfil/onboarding e pré-preencher; (b) usar **boleto/Pix como meio principal**, onde a cobrança usa o cliente que NÓS criamos, com o CNPJ, e o problema não existe; (c) emitir a nota fora do fluxo, à mão, como é hoje na Stripe.
  - ✅ **SIGN-OFF FEITO (31/07/2026):** o dono validou a tela no navegador. A task fecha com o critério do §4.4 cumprido — não sobrou dívida de evidência.
...
- [~] **T-217 — Cancelamento self-service** 🟠 — **BACKEND + TELA FEITOS (31/07/2026, `7f554b4`). Falta só o sign-off no navegador (§4.4).**
  - 🔴 **A medição veio primeiro, e derrubou a premissa da task: NÃO EXISTE "cancelar no fim do período" no Asaas.** `DELETE /v3/subscriptions/{id}` é imediato e terminal. O "acesso até o fim do que foi pago" é **100% regra nossa** — e ela **já existia**: `calcularAcesso` libera `CANCELED` enquanto `currentPeriodEnd` estiver no futuro (T-144). **Zero linha alterada em `acesso.ts`.** Por isso o cancelamento **não toca** nessa data: apagá-la cobraria um mês e entregaria meio.
  - 🔴 **A lição da T-144 se repetiu, de roupa nova: são DOIS sinais.** Depois do DELETE, o GET ainda responde e traz **`deleted: true` E `status: "INACTIVE"`**. E a assinatura **some do `GET /subscriptions`** — só o GET direto por id a encontra. ⚠️ **Isso é armadilha para a T-223:** uma reconciliação que *lista* as assinaturas do Asaas não vê as canceladas e pode concluir "sumiu, logo divergiu".
  - ✅ **DECISÃO DO DONO (31/07): a cobrança em aberto é apagada junto, e tudo bem.** Medido: `/payments` foi de 1 para zero na cobrança PENDENTE; a cobrança **já paga (CONFIRMED) sobrevive**. A razão que sustenta a decisão é do dono e vale mais que a medição: **a cobrança é PRÉ-PAGA — financia o ciclo que vem, não o que já foi usado.** Cancelar antes de pagá-la não perdoa dívida de consumo, cancela um adiantamento. ⚠️ **Não "conserte" recriando a cobrança como avulsa:** seria cobrar quem já saiu, por um período que não vai usar.
  - ⚠️ **A ORDEM É PROVEDOR PRIMEIRO, BANCO DEPOIS** — as duas falhas não são simétricas. Banco cancelado + provedor ativo = corta o acesso **e segue cobrando todo mês**, o pior resultado possível; provedor cancelado + banco intacto = ninguém é cobrado e a reconciliação conserta. Há teste travando a ordem.
  - 🔴 **BURACO FECHADO DE PASSAGEM, e ele só apareceu porque rodei contra o banco de dev:** a assinatura Asaas **ativa** estava com `current_period_end` **NULO** — e sem essa data o cancelamento cortaria o acesso **na hora**, quebrando exatamente a promessa da task. Quem preenche no caminho normal é o webhook (`fimDoPeriodo`), e ela fica nula quando aquela leitura falha na **primeira** cobrança. Agora o fim do período é lido do provedor **antes** do DELETE. ⚠️ **Só para quem está `ACTIVE`**: fazer isso num `past_due` daria acesso pago a quem não pagou (o `nextDueDate` do inadimplente também aponta para o futuro) — quem manda lá é a carência.
  - ✅ **Motivo coletado, em lista FECHADA** (`motivos-cancelamento.ts`) + campo livre opcional (500 no DTO **e** na coluna). Texto livre puro viraria 40 respostas para a mesma coisa e não se agrupa. **Obrigatório**: perguntar depois, por e-mail, tem resposta perto de zero.
  - ⚠️ **A auditoria NÃO foi para o `admin_audit_log`, e isso é decisão:** aquela tabela exige `admin_user_id` e é a trilha de **ações de admin** — gravar o id do cliente ali forjaria um admin que não existiu. O motivo aparece no **detalhe da conta em `/admin`**, que já devolvia o estado da assinatura.
  - 📌 **Os campos ficam na ASSINATURA, não em tabela própria (decisão do dono):** quem cancelar, voltar e cancelar de novo **sobrescreve** o motivo anterior. Perda de histórico consciente — a conta é 1:1 com o usuário e o volume do beta não paga entidade + tela novas.
  - ✅ **VALIDADO PONTA A PONTA contra o sandbox (31/07)**, não só build verde: `POST /assinaturas/cancelar` → **200** com `acessoAte: 2026-09-30T03:00:00Z` (meia-noite de **Brasília**, o fuso certo); `/users/me` seguindo com **`acessoPermitido: true`** e `status: canceled` — a regra da T-144 provada no caminho real; 2ª chamada **idempotente**, sem reescrever o motivo nem falar com o provedor; DTO barrando motivo ausente, motivo inventado e detalhe >500 **antes** de sair para o Asaas; e o **e-mail falhando sem derrubar o cancelamento** (o Resend de dev recusa destinatário externo — o 200 veio assim mesmo, que é a garantia do `emSegundoPlano`).
  - **Migration `up`/`down`/`up` testados contra Postgres real.** API **1044** testes verdes (+14), front 149, lint limpo nos dois.
  - ✅ **Estado cancelado não oferece o que não existe, e reativar NÃO cobra duas vezes (31/07, `bbaf50e`) — achado do dono na tela.** Cancelada, os botões de **trocar cartão** e **trocar plano** somem: não há assinatura no provedor para alterar (ela foi apagada), e botão que não faz nada é pior que botão ausente.
    - 🔴 **A armadilha que isso destapou tinha dinheiro dentro:** os dois caminhos de criação usavam `nextDueDate: hojeISO()`. Como quem cancelou **segue com acesso** até o fim do que pagou (T-144), um "Reativar" ingênuo criaria assinatura cobrando **hoje** — pelo mês que a pessoa **já pagou e ainda está usando**. Seria o terceiro bug de cobrança duplicada do épico.
    - **A data da 1ª cobrança agora é decidida no SERVIDOR** (`primeiroVencimento`, §3.3): cancelada e ainda dentro do período pago → vence **no fim desse período**; senão, hoje. Data de cobrança é dinheiro — cliente que a escolhesse escolheria "nunca".
    - ✅ **Medido antes de escrever:** `nextDueDate` futuro é aceito pelos **dois** caminhos (checkout hospedado **e** assinatura direta), e a cobrança nasce vencendo na data pedida. ✅ **E validado ponta a ponta:** cancelei com acesso até 31/08, reativei por boleto/Pix, e a cobrança nova venceu **31/08** — não hoje. Sem cobrança dupla e sem buraco no acesso.
    - ⚠️ **`dataBrasiliaISO` nasceu aqui, e a razão vale além dela:** `toISOString().slice(0,10)` num servidor UTC (§8) erra o dia para instantes da noite — e um dia de erro aqui é um dia de **cobrança adiantada**. 📌 O `hojeISO()` antigo tem o mesmo defeito latente (a partir das 21h de Brasília ele já diz "amanhã"); não mexi por escopo (§4.3), mas é dívida registrada.
  - 🔴 **DOIS defeitos achados pelo dono NA TELA (31/07), e os dois eram invisíveis em teste:**
    1. **"Trocar de plano" continuava aparecendo numa assinatura cancelada.** Passar `onTrocarPlano: undefined` não bastava: o botão do `AssinanteCard` renderizava **sempre**, caindo em `onTrocarPlano ?? onPortal`. Resultado: um botão **morto** — pior que botão nenhum. Agora ele só existe com handler. 📌 **Lição:** esconder um botão por *props opcionais* só funciona se o componente respeitar a ausência; conferir o consumidor, não só o chamador.
    2. 🔴 **Quem cancelava e voltava ficava marcado como cancelado PARA SEMPRE.** **Nada** limpava o `cancelAtPeriodEnd` — nem o webhook, nem a criação da assinatura nova. A tela seguia dizendo "Cancelada · acesso até X" e oferecendo *reativar* a quem já tinha pago. Corrigido no **webhook** (a fonte da verdade): pagamento confirmado desmarca a flag. ⚠️ **O motivo e a data do cancelamento FICAM** — são histórico de churn, não estado; apagá-los na volta destruiria a única leitura de "por que estão saindo" que o beta tem.
  - 🔴 **A REATIVAÇÃO NÃO CONFIRMAVA — e o CHECKOUT HOSPEDADO FOI REMOVIDO por causa disso (01/08, decisão do dono).**
    - **O que o dono viu:** clicou "Reativar", e a tela continuou dizendo "Cancelada". **A reativação tinha funcionado no Asaas** — a assinatura existia lá, com a 1ª cobrança `PENDING` para **31/08/2027**, porque o acesso pago ia até lá.
    - **A causa é minha:** eu agendei a 1ª cobrança para o fim do período pago (certo — é o que evita cobrar duas vezes) **mas deixei o sistema depender de um `PAYMENT_CONFIRMED` para saber que a pessoa voltou**. O pagamento foi adiado de propósito, então o evento só chegaria um ano depois. 📌 **Lição: não faça o estado depender de um evento que você mesmo adiou.**
    - ✅ **Corrigido nos dois caminhos:** reativar dentro do período pago devolve a assinatura a `active` **na hora** (`reativando`). ⚠️ Só para quem **tem período pago em aberto** — fora disso quem libera continua sendo o webhook, senão seria produto de graça.
    - ✅ **E o checkout hospedado saiu de cena.** Ele causou **quatro** problemas, todos da mesma raiz — *criava cliente e assinatura por conta própria e nós só descobríamos depois*: cliente fantasma, assinatura duplicada cobrando em paralelo, **CPF do pagador em vez do CNPJ na nota** e esta reativação. Agora a assinatura de cartão é criada por **nós** (`POST /subscriptions` com `creditCard`), com o **nosso** cliente.
    - ✅ **Isso FECHA o risco aberto da T-219:** medido em dev — o cliente da assinatura nova tem o **CNPJ da empresa**, não o CPF do pagador. Era o bloqueio que o §9 registrava como "precisa de decisão do dono".
    - ⚠️ **PCI não aumentou:** o cartão já passava pelo nosso servidor desde 31/07 (SAQ A-EP). **Mas a task de conformidade que o critério da T-207 exigia continua não existindo** — ASV trimestral, política de logs, resposta a incidente. Não bloqueia código; bloqueia faturar.
    - ✅ **MEDIDO no sandbox antes de escrever** (é o que evitou o próximo susto): o Asaas **valida o cartão na criação mesmo com vencimento futuro** — expirado e recusa do emissor voltam 400 na hora. ⚠️ **Mas não pega tudo: número que falha no Luhn foi aceito.** A validação do formulário (`lib/cartao.ts`) é **carga, não enfeite**.
    - ✅ **Validado ponta a ponta em dev:** cancelada → `POST /assinaturas/assinar-cartao` → **200** com só `{ultimos4, bandeira}`; `/users/me` virou **`active`** na hora; a assinatura no Asaas ficou com o **nosso cliente + CNPJ**, **sem `checkoutSession`**, e a 1ª cobrança em **15/09** (o fim do período pago), não hoje; e `grep` do número e do CVV em todos os logs da API → **zero**.
    - 📌 **Sobrou lixo no sandbox** (5 assinaturas de teste, 4 delas em R$ 20.000 de uma calibragem de preço antiga, 1 criada pelo checkout removido). Não apaguei sem perguntar.
  - ⚠️ **Falta (§4.4): o clique humano.** Fixture pronto em dev — `docker@teste.com` está com `provider='asaas'`, assinatura de sandbox **ativa** e uma cobrança em aberto de R$ 249. *(Ele ganhou um CNPJ no caminho: sem isso o `/checkout` recusa, por causa da T-225.)*
  - 📌 **Achado que a T-222 herda:** o rodapé da tela de assinatura diz *"O pagamento é processado pela Stripe"* — mentira para quem é do Asaas. Não corrigi aqui por escopo (§4.3); é exatamente o que a T-222 existe para varrer.
  - **Pronto quando:** ~~cancelamento reflete no Asaas e na base; acesso permanece até o fim do ciclo pago; motivo aparece no `/admin`~~ — os três feitos e medidos; falta o sign-off.
