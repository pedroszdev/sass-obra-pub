...
- [x] **T-205 — Cloudflare Access na frente do `/admin`** 🟠 (segurança) — **FEITO em 29/07/2026, AS DUAS ETAPAS. Nenhuma linha de código.**
  ...
  - ✅ **Sessão entre os dois apps:** um código só. Autenticar no `app` cobre o `api` sem novo PIN (medido pelo dono). ❓ **Não medido:** se visitar `api…/admin/*` numa navegação de topo é *necessário* para os XHR do painel funcionarem, ou se o cookie já vem sem isso. Se for necessário, existe um passo manual a cada 8h; se não, não existe atrito. Testável em 30s numa anônima nova, indo direto do código para as telas do painel.
