...
- [~] **T-216 — Portal do assinante** 🔴 — **BACKEND FEITO em 30/07/2026; a TELA é o que falta.** *(o item mais caro do épico)*
  ...
  - **Falta (a maior parte do esforço):** a **tela** em React/Mantine — plano atual, valor e próximo vencimento, histórico de cobranças com status, botão de pagar a cobrança em aberto (leva ao `invoiceUrl`), e as ações de cartão **redirecionando para um checkout hospedado novo** (não há caminho PCI-limpo por API, T-207). Alterar plano (mensal ↔ anual) com regra de proporcional **ainda não definida**.
